...
# The paths KitabuCode loads by name. server.js:57 forks the first, rewrites the
...
def main(tree):
    ...
    if product is not None:
        check(product.get("nameLong") == "KitabuCode", "product.json is branded",
    ...
    if css is not None:
        check("KitabuCode: Mobile-friendly" in css,
    ...
    manifest = present(tree / "kitabucode-patches.json", "kitabucode-patches.json")
    if manifest:
        ...
        print("  note    no kitabucode-patches.json, and ALLOW_UNADAPTED is set: "
        ...
        check(False, "the tree records which patches built it",
              "no kitabucode-patches.json at the tree root, so this tree predates "
    ...
    # KitabuCodeWebViewClient's shouldInterceptRequest answers locally; the

...
function notificationFor(snapshot) {
    const tmp = fs.mkdtempSync(path.join(BASE_TMP, 'kitabucode-ext-'));
    process.env.TMPDIR = tmp;
    fs.writeFileSync(path.join(tmp, 'kitabucode-processes.json'), JSON.stringify(snapshot));
...
{
    const tmp = fs.mkdtempSync(path.join(BASE_TMP, 'kitabucode-latch-'));
    process.env.TMPDIR = tmp;
    delete require.cache[require.resolve(EXTENSION)];
    const extension = require(EXTENSION);
    const poll = (snap) => {
        fs.writeFileSync(path.join(tmp, 'kitabucode-processes.json'), JSON.stringify(snap));
        shown.length = 0;
        extension.activate({ subscriptions: [] });
        extension.deactivate();
        return shown.slice();
...
{
    const tmp = fs.mkdtempSync(path.join(BASE_TMP, 'kitabucode-tier-'));
    process.env.TMPDIR = tmp;
    delete require.cache[require.resolve(EXTENSION)];
    const extension = require(EXTENSION);
    const poll = (snap) => {
...
// never returns.
{
    const tmp = fs.mkdtempSync(path.join(BASE_TMP, 'kitabucode-rearm-'));
    process.env.TMPDIR = tmp;
    delete require.cache[require.resolve(EXTENSION)];
    const extension = require(EXTENSION);
    const poll = (snap) => {
...
// that exists.
//
// NEGATIVE CONTROL: register `kitabucode.killIdleServers` again and the first
// assertion goes red before anything is signalled.
{
    ...
    try {
        ...
        assert.ok(commands.has('kitabucode.showProcesses'), 'the details view command is gone');
        assert.deepStrictEqual(
            [...commands.keys()].filter((id) => /kill|idle/i.test(id)), [],
            `a command that signals language servers is registered again: ${[...commands.keys()]}`,
        );
        commands.get('kitabucode.showProcesses')();
    } finally {

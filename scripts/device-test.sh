...
derive_toolchain_release_urls() {
    ...
    sed -n 's/.*downloadUrl = "\(https:[^"]*\)".*/\1/p' \
        "$ROOT_DIR/android/app/src/main/kotlin/com/kitabucode/setup/ToolchainRegistry.kt"
}
...
derive_app_path() {
    ...
    sed -n "s|^ *\"$1\" to \"\\\$filesDir/\([^\"]*\)\",\$|\1|p" \
        "$ROOT_DIR/android/app/src/main/kotlin/com/kitabucode/util/Environment.kt" | head -1
}
...
derive_shortcut_target() {
    ...
    awk '/private fun publishToolchainShortcut/, /^    }$/' \
        "$ROOT_DIR/android/app/src/main/kotlin/com/kitabucode/SplashActivity.kt" \
...
derive_ready_budget_seconds() {
    ...
    local pm="$ROOT_DIR/android/app/src/main/kotlin/com/kitabucode/service/ProcessManager.kt"
    local ns="$ROOT_DIR/android/app/src/main/kotlin/com/kitabucode/service/NodeService.kt"
...
' "$ROOT_DIR/android/app/src/main/kotlin/com/kitabucode/setup/ToolchainManager.kt"
}
...
# ── Header ─────────────────────────────────────────────────────────
printf "\n${BOLD}${CYAN}=== KitabuCode Device Test Suite ===${RESET}\n\n"
...
LAUNCH_OUT=$($ADB shell am start -n "$PKG/com.kitabucode.SplashActivity" 2>&1)
...
# On first run: KitabuCode.FirstRunSetup logs "First-run setup completed"
# On subsequent runs: KitabuCode.SplashActivity logs "Not first run, launching main"
SETUP_TIMEOUT=$TIMEOUT
ELAPSED=0
SETUP_OK=false

while [ $ELAPSED -lt $SETUP_TIMEOUT ]; do
    LOGCAT=$($ADB logcat -d -s KitabuCode.SplashActivity:I KitabuCode.FirstRunSetup:I 2>/dev/null)
...
while [ -n "$READY_TIMEOUT" ] && [ $ELAPSED -lt "$READY_TIMEOUT" ]; do
    LOGCAT=$($ADB logcat -d -s KitabuCode.NodeService:I KitabuCode.ProcessManager:I 2>/dev/null)
...
EXT_JSON=$($ADB shell run-as "$PKG" cat "files/home/.kitabucode/extensions/extensions.json" 2>/dev/null || true)
...
SETTINGS_CHECK=$($ADB shell "run-as $PKG sh -c 'test -f files/home/.kitabucode/data/Machine/settings.json && echo EXISTS'" 2>/dev/null)
...
TEST_FILE="files/home/.kitabucode-test-$$"
...
# saf-bridge extension's kitabucode.manageToolchains command, which sends
...
TC_STATE=$($ADB shell run-as "$PKG" cat files/home/.kitabucode/toolchains.json 2>/dev/null | tr -d '\r')
TC_NAMES=""
TC_STATE_OK=false
if [ -n "$TC_STATE" ]; then
...
if [ -z "$TC_STATE" ]; then
    ...
    TC_ENV=$($ADB shell run-as "$PKG" cat files/home/.kitabucode/toolchain-env.sh 2>/dev/null | tr -d '\r')
    if [ -n "$TC_ENV" ]; then
...
TERM_PROFILE_PATH=""
SETTINGS_JSON=$($ADB shell run-as "$PKG" cat files/home/.kitabucode/data/Machine/settings.json 2>/dev/null | tr -d '\r')
...
PARSE_FAILURES=""
PARSE_CHECKED=0
for rc_file in files/home/.bashrc files/home/.kitabucode/toolchain-env.sh; do

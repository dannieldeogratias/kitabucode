...
named after the applicationId, so it is `com.kitabucode.DYNAMIC_RECEIVER_...` in
a release build and `com.kitabucode.debug.DYNAMIC_RECEIVER_...` in a debug one,
...
_POLICY_CONTROLS = (
    ...
    ("an applicationId-scoped one", "`com.kitabucode.DYNAMIC_RECEIVER_X`",
     {"DYNAMIC_RECEIVER_X"}),

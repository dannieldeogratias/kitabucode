...
const PROCESS_MANAGER = path.resolve(
    __dirname, '../android/app/src/main/kotlin/com/kitabucode/service/ProcessManager.kt',
);
...
function fixture(productJson, { serverMain = 'process.exit(0);\n', dnsProxy = false } = {}) {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'kitabucode-bootstrap-'));
...
// 1. A valid file is rewritten with the overrides, and nothing is left beside it.
{
    ...
    assert.strictEqual(product.nameShort, 'KitabuCode', 'the overrides were not applied');
...
async function preloadRidesAsOneToken() {
    const dnsProxy = [
        ...
        "if (process.env.KITABUCODE_DNS_PROXY === '1') {",
...
async function proxySurvivesTheBootstrap() {
    ...
    try {
        ...
        assert.match(
            report.proxy,
            /^http:\/\/kitabucode:[0-9a-f]+@127\.0\.0\.1:\d+$/,

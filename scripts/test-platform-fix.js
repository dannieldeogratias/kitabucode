...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'kitabucode-platform-'));
...
const PROBE = "console.log(JSON.stringify({ platform: process.platform, optIn: process.env.KITABUCODE_PLATFORM_FIX || null }));\n";
...
const cases = [
    ...
    ['the npm opt-in', `${tmp}/home/install.js`, [], { KITABUCODE_PLATFORM_FIX: '1' }, 'linux'],
];
...
for (const [label, script, args, env, want] of cases) {
    ...
    assert.strictEqual(got.optIn, null, `${label}: KITABUCODE_PLATFORM_FIX survived into the process`);
    checked++;
}

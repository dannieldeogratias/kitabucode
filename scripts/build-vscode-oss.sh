...
# trusting a local green, `docker volume rm kitabucode-codeoss`.
#
...
#       -t kitabucode-codeoss-build:24.18.0 docker/
#   docker volume create kitabucode-codeoss
#   docker volume create kitabucode-npm-cache
#   docker run --rm -v kitabucode-codeoss:/work \
#       -v kitabucode-npm-cache:/root/.npm \
#       -e VSCODE_VERSION="$(cat VSCODE_VERSION)" \
#       -e VSCODE_COMMIT="$(cat VSCODE_COMMIT)" \
#       -v "$PWD/scripts:/scripts:ro" \
#       -v "$PWD/branding:/branding" \
#       -v "$PWD/patches:/patches:ro" \
#       kitabucode-codeoss-build:24.18.0 bash /scripts/build-vscode-oss.sh
#
...
/* KitabuCode: Mobile-friendly hamburger menu overrides */
...
CSSEOF
grep -q "KitabuCode: Mobile-friendly" "$WORKBENCH_CSS" || { echo "  ERROR: append did not land" >&2; exit 1; }
...
check("manifest.json is branded", manifest.get("name") == "KitabuCode",

...
const EXTENSION = path.join(
    newestExtensionDir('kitabucode.kitabucode-welcome-'), 'extension.js',
);
...
const MARKER = '.kitabucode_welcome_shown';
...
async function activate(home, commandFor) {
    ...
    return {
        ...
        aligned: workspaceState.get('kitabucode.secondarySideBar.aligned') === true,
        ran: ran.slice(),
    };
}
...
async function main() {
    const base = fs.mkdtempSync(path.join(os.tmpdir(), 'kitabucode-welcome-'));
    try {
        ...
        const open = registered.get('kitabucode.welcome.open');
        assert.ok(open, 'the extension no longer registers kitabucode.welcome.open');

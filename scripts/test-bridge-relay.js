...
const MAIN_ACTIVITY = path.join(
    ROOT, 'android/app/src/main/kotlin/com/kitabucode/MainActivity.kt',
);
const ANDROID_BRIDGE = path.join(
    ROOT, 'android/app/src/main/kotlin/com/kitabucode/bridge/AndroidBridge.kt',
);
...
function bridgeExtension() {
    return path.join(
        newestExtensionDir('kitabucode.kitabucode-saf-bridge-'), 'extension.js',
    );
}
...
async function main() {
    ...
    const relayWindow = { __kitabucode: { authToken: 'test-token' } };
    ...
    assert.strictEqual(
        typeof relayWindow.__kitabucodeBridgeReply, 'function',
    ...
    const openInBrowser = commands.get('kitabucode.openInBrowser');
    assert.ok(openInBrowser, 'the bundled extension no longer registers kitabucode.openInBrowser');
    ...
    function windowOpen(answer, url) {
        ...
        const win = {
            __kitabucode: { authToken: 'test-token' },
    ...
    // answer fails.
    const manageDeviceFolders = commands.get('kitabucode.manageDeviceFolders');
    assert.ok(manageDeviceFolders,
        'the bundled extension no longer registers kitabucode.manageDeviceFolders');
    ...
    relayWindow.__kitabucodeBridgeReply(
    ...
    await settle();
    relayWindow.__kitabucodeBridgeReply(listCalls[0].replyId, false, POSTED_ONLY_HERE);
    ...
    relayWindow.__kitabucodeBridgeReply(reclaimCalls[0].replyId, false, IN_USE);
    ...
    const showStorageUsage = commands.get('kitabucode.showStorageUsage');
    assert.ok(showStorageUsage, 'the bundled extension no longer registers kitabucode.showStorageUsage');

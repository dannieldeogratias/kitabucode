...
async function main() {
    ...
    const wrongToken = await proxiedGet(proxyPort, `http://127.0.0.1:${originPort}/`, 'kitabucode:not-the-token');
    ...
    const duplicated = await rawExchange(
        proxyPort,
        `GET http://127.0.0.1:${originPort}/ HTTP/1.1\r\nHost: 127.0.0.1:${originPort}\r\n` +
            `Proxy-Authorization: Basic ${Buffer.from('kitabucode:wrong').toString('base64')}\r\n` +
            `Proxy-Authorization: Basic ${Buffer.from(goodCredentials).toString('base64')}\r\n\r\n`,
    );
    ...
    const badTunnel = await rawExchange(
        proxyPort,
        `${connectHead}Proxy-Authorization: Basic ${Buffer.from('kitabucode:wrong').toString('base64')}\r\n\r\n`,
    );

...
# Launch
echo ""
echo "Launching KitabuCode..."
# The debug build type sets applicationIdSuffix = ".debug", so the package this
# script just installed is com.kitabucode.debug. Launching com.kitabucode resolved
# nothing ("Activity class {com.kitabucode/com.kitabucode.SplashActivity} does not
...
PKG="${PKG:-com.kitabucode.debug}"
adb shell am start -n "$PKG/com.kitabucode.SplashActivity"
echo "  ✓ Launched $PKG"

echo ""
echo "=== Deploy complete ==="
# Every tag is KitabuCode.<class>, so the bare tag matches nothing as a filterspec,
# and logcat filterspecs take no wildcard.
echo "View logs: adb logcat | grep KitabuCode"

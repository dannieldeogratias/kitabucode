...
val bundleNotices = tasks.register<Sync>("bundleNotices") {
    ...
    // com.kitabucode.util.Notices lists the same six, and NoticesTest compares
...
android {
    ...
    signingConfigs {
        create("release") {
            storeFile = file(signingProp("storeFile", "KITABUCODE_KEYSTORE_FILE",
                "${System.getProperty("user.home")}/kitabucode-release.jks"))
            storePassword = signingProp("storePassword", "KITABUCODE_KEYSTORE_PASSWORD")
            keyAlias = signingProp("keyAlias", "KITABUCODE_KEY_ALIAS", "kitabucode")
            keyPassword = signingProp("keyPassword", "KITABUCODE_KEY_PASSWORD")
        }
    }
    ...
    buildTypes {
        ...
        debug {
            ...
            enableUnitTestCoverage = providers.gradleProperty("kitabucodeCoverage").isPresent
        }
    }
...
tasks.withType<Test> {
    ...
    // `<application android:name>` still points at KitabuCodeApp, the class that
...
val checkPatchFingerprints = tasks.register<Exec>("checkPatchFingerprints") {
    ...
    failOnExit(
        "The server tree in assets/ is missing at least one patch this checkout applies.\n" +
            ...
            "      docker volume rm kitabucode-codeoss\n" +

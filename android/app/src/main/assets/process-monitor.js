...
function start(options) {
    ...
    outputPath = path.join(tmpDir, 'kitabucode-processes.json');
...
function classify(cmdline) {
    ...
    // name in their binary PATH (e.g. /data/app/.../com.kitabucode.debug-.../lib/...)
    // so we must check for exact match, not substring.
    if (/^com\.kitabucode(\.\w+)?$/.test(cmd)) return 'app';

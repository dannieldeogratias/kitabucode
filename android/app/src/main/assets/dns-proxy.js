 ...
 * appear to come from KitabuCode. The random port only raises the cost of
...
function start(log) {
    ...
    const expected = crypto
        .createHash('sha256')
        .update(Buffer.from(`kitabucode:${token}`).toString('base64'))
        .digest();
    ...
    const CHALLENGE = 'Basic realm="kitabucode"';
    ...
    return new Promise((resolve) => {
        ...
        server.listen(0, '127.0.0.1', () => {
            ...
            const url = `http://kitabucode:${token}@127.0.0.1:${port}`;
            done({
                HTTP_PROXY: url,
                HTTPS_PROXY: url,
                http_proxy: url,
...
if (process.env.KITABUCODE_DNS_PROXY === '1') {
    delete process.env.KITABUCODE_DNS_PROXY;

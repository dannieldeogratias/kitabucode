...
/**
 * KitabuCode SAF Bridge Extension (Browser)
 *
 ...
 * main page, where it listens on BroadcastChannel 'kitabucode-bridge' and
 * forwards commands to the AndroidBridge JavascriptInterface.
 *
 * Commands:
 * - kitabucode.openFolderFromDevice : Opens SAF folder picker
 * - kitabucode.openRecentFolder     : Shows Quick Pick of recently opened folders
 * - kitabucode.openInBrowser        : Opens a URL in the device browser
 * - kitabucode.generateSshKey       : Creates ~/.ssh/id_ed25519
 * - kitabucode.copySshPublicKey     : Copies the public key to the clipboard
 * - kitabucode.showStorageUsage     : Per-component storage breakdown
 * - kitabucode.manageDeviceFolders  : Lists the local copies of device folders and removes one
 * - kitabucode.clearCaches          : Deletes cached data, reports bytes freed
 * - kitabucode.manageToolchains     : Opens the Android Toolchains screen
 * - kitabucode.about                : Opens the Android About dialog
 */
...
function getChannel() {
    ...
    _channel = new BroadcastChannel('kitabucode-bridge');
...
function activate(context) {
    ...
    const openFolderCmd = vscode.commands.registerCommand(
        'kitabucode.openFolderFromDevice',
        async () => {
            try {
            ...
            try {
                ...
                if (folders.length === 0) {
                    ...
                    if (action === 'Open Folder') {
                        vscode.commands.executeCommand('kitabucode.openFolderFromDevice');
                    }
                    return;
                }
            ...
            try {
                ...
                if (result.success) {
                    ...
                    vscode.window.showInformationMessage(
                        result.existed
                            ? 'An SSH key is already set up at ~/.ssh/id_ed25519. Run "KitabuCode: Copy SSH Public Key" to add it to your Git host.'
                            : 'SSH key created at ~/.ssh/id_ed25519. Run "KitabuCode: Copy SSH Public Key" to add it to your Git host.'
                    );
                } else {
                    vscode.window.showErrorMessage(
    ...
    const copySshPublicKeyCmd = vscode.commands.registerCommand(
        'kitabucode.copySshPublicKey',
        async () => {
            try {
                const pubKey = /** @type {string} */ (await sendBridgeCommand('getSshPublicKey'));
                if (!pubKey || !pubKey.trim()) {
                    vscode.window.showWarningMessage(
                        'No SSH key yet. Run "KitabuCode: Generate SSH Key" first.'
                    );
                    return;
                }
        ...
        async () => {
            try {
                ...
                if (picked.key && clearable.has(picked.key)) {
                    vscode.commands.executeCommand('kitabucode.clearCaches');
                    ...
                    vscode.commands.executeCommand('kitabucode.manageDeviceFolders');
                } else if (picked.key) {
                    vscode.window.showInformationMessage(
    ...
    const manageDeviceFoldersCmd = vscode.commands.registerCommand(
        'kitabucode.manageDeviceFolders',
        async () => {
            /** @type {*[]} */
            let mirrors;
            ...
            try {
                ...
                if (mirrors.length === 0) {
                    vscode.window.showInformationMessage(
                        'No device folders have been copied into KitabuCode yet.'
                    );
                    return;
                }
            } catch (/** @type {*} */ err) {
            ...
            try {
                ...
                const message = m.reclaimable
                    ? `Remove KitabuCode's local copy of ${name} and free ${size}? ` +
    ...
    const aboutCmd = vscode.commands.registerCommand('kitabucode.about', async () => {
        try {

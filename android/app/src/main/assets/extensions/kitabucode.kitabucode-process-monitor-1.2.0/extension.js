...
function activate(context) {
    ...
    const snapshotPath = path.join(tmpDir, 'kitabucode-processes.json');

    // Status bar item: right side, low priority (far right)
    statusBarItem = vscode.window.createStatusBarItem(
        vscode.StatusBarAlignment.Right, -100
    );
    statusBarItem.command = 'kitabucode.showProcesses';
    statusBarItem.name = 'Process Monitor';
    statusBarItem.text = '$(pulse) --';
    statusBarItem.tooltip = 'KitabuCode Process Monitor: loading...';
    statusBarItem.show();

    outputChannel = vscode.window.createOutputChannel('KitabuCode Processes');

    // Register commands
    const showCmd = vscode.commands.registerCommand('kitabucode.showProcesses', () => {
        showProcessTree();
    });
    ...
    function poll() {
        ...
        if (lastSnapshot && isStale(lastSnapshot)) {
            ...
            statusBarItem.tooltip =
                `KitabuCode Process Monitor: no process data for ${STALE_AFTER_MS / 1000} s, ` +
...
function showProcessTree() {
    ...
    outputChannel.appendLine(`KitabuCode Process Tree (${time})`);

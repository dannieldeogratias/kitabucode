...
import com.kitabucode.util.drawBehindSystemBars
import com.kitabucode.util.CrashReporter
import com.kitabucode.util.StorageManager
import com.kitabucode.util.WebViewVersion
...
import com.kitabucode.util.Environment
import com.kitabucode.bridge.AUTH_TAB_WINDOW_MILLIS
import com.kitabucode.bridge.AndroidBridge
import com.kitabucode.bridge.AuthTabWindow
import com.kitabucode.bridge.ClipboardBridge
import com.kitabucode.bridge.SecurityManager
import com.kitabucode.keyboard.ExtraKeyRow
import com.kitabucode.keyboard.KeyInjector
import com.kitabucode.service.NodeService
import com.kitabucode.service.StartupNotice
import com.kitabucode.setup.FirstRunSetup
import com.kitabucode.storage.SafFolderInfo
import com.kitabucode.storage.SafStorageManager
import com.kitabucode.util.Logger
import com.kitabucode.util.MainThreadWatch
import com.kitabucode.util.Notices
import com.kitabucode.webview.DownloadCoordinator
import com.kitabucode.webview.DownloadHost
import com.kitabucode.webview.DownloadOutcome
import com.kitabucode.webview.KitabuCodeWebChromeClient
import com.kitabucode.webview.KitabuCodeWebView
import com.kitabucode.webview.KitabuCodeWebViewClient
import com.kitabucode.webview.RETRY_URL
import com.kitabucode.webview.TlsFailure
import com.kitabucode.webview.TlsFailureReason
import com.kitabucode.webview.HandoffFailure
import com.kitabucode.webview.handoffFailureToAnnounce
import com.kitabucode.webview.publishedResourceRoots
import com.kitabucode.webview.redactToken
import com.kitabucode.webview.sensitiveLocations
import com.kitabucode.webview.tlsFailureToAnnounce
...
class MainActivity : AppCompatActivity() {
     ...
     * [KitabuCodeWebViewClient] on every cycle, so a counter held there is reset by
    ...
    private fun deliverFileChooserResult(uris: List<Uri>) {
        val client = webView?.webChromeClient as? KitabuCodeWebChromeClient
        if (client == null) {
    ...
    private val downloads: DownloadCoordinator = DownloadCoordinator(object : DownloadHost {
        ...
        override fun requestBytes(requestId: String, url: String) {
            ...
            val script = "(function() {" +
                "  if (!window.__kitabucodeDownload) return false;" +
                "  return window.__kitabucodeDownload.send(" +
        ...
        override fun releaseBytes(url: String) {
            val script = "(function() {" +
                "  var d = window.__kitabucodeDownload;" +
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // Its own wording, because "the only copy is inside KitabuCode" is the opposite of
        ...
        // the mirror copy is gone, so "the only copy is inside KitabuCode" would be
     ...
     * there. The returning `kitabucode://callback` then builds a *new*
    ...
    override fun onTrimMemory(level: Int) {
        ...
        webView?.evaluateJavascript(
            "window.__kitabucode?.onLowMemory?.($level)", null
        )
    }
    ...
    private fun answerBridgeCommand(id: String, ok: Boolean, payload: String) {
        val script = "if (window.__kitabucodeBridgeReply) window.__kitabucodeBridgeReply(" +
     ...
     *   falsy value; see [com.kitabucode.bridge.AndroidBridge.reclaimSafMirror].
     */
    ...
    private fun fileChooserIsPending(): Boolean =
        (webView?.webChromeClient as? KitabuCodeWebChromeClient)?.hasPendingFileChooser == true
    ...
    private fun setupWebView() {
        ...
        webView?.let { wv ->
            KitabuCodeWebView.configure(wv)
     ...
     * ends the application process. [KitabuCodeWebViewClient] returns true and
    ...
    private fun loadingPage(): String =
        """<html><head><meta name="viewport" content="width=device-width, initial-scale=1.0, viewport-fit=cover"></head>
           ...
           <div style="text-align:center"><h2 style="color:#ccc;">KitabuCode</h2>
     ...
     * `KitabuCodeWebViewClient`, because the server can give up at any point in the
    ...
    private fun showErrorPage(message: String, control: String) {
        ...
        webView?.loadDataWithBaseURL(
            ...
            """<html><head><meta name="viewport" content="width=device-width, initial-scale=1.0, viewport-fit=cover"></head>
               ...
               <h2 style="color:#ccc;margin:0 0 .6em">KitabuCode</h2>
    ...
    private fun initBridge(port: Int) {
        ...
        KitabuCodeWebViewClient.setupServiceWorkerInterception(
            port, roots, sensitive, { self.get()?.openWorkspaceFolder }
        ) { self.get()?.nodeService?.getConnectionToken() }

        wv.webViewClient = KitabuCodeWebViewClient(
            allowedPort = port,
            resourceRoots = roots,
        ...
        wv.webChromeClient = KitabuCodeWebChromeClient { allowMultiple ->
    ...
    private fun injectBridgeToken() {
        ...
        webView?.evaluateJavascript(
            "window.__kitabucode = window.__kitabucode || {}; window.__kitabucode.authToken = '$token';",
            null
        )
    ...
    private fun injectSafeAreaCSS() {
        webView?.evaluateJavascript(
            """
            (function() {
                if (document.getElementById('kitabucode-safe-area-css')) return;
                ...
                style.id = 'kitabucode-safe-area-css';
                style.textContent = [
                    '/* KitabuCode: Safe area padding for round-corner devices */',
                    '.part.activitybar {',
    ...
    private fun injectTouchTargetCSS() {
        webView?.evaluateJavascript(
            """
            (function() {
                if (document.getElementById('kitabucode-touch-css')) return;
                var s = document.createElement('style');
                s.id = 'kitabucode-touch-css';
                s.textContent = [
                    '/* KitabuCode: Enlarged touch targets for touch input */',
    ...
    private fun injectWindowOpenOverride() {
        webView?.evaluateJavascript(
            """
            (function() {
                if (window.__kitabucodeOpenPatched) return;
                window.__kitabucodeOpenPatched = true;
                var orig = window.open;
                window.open = function(url) {
                    if (url && /^https?:/.test(url) && typeof AndroidBridge !== 'undefined') {
                        var t = (window.__kitabucode || {}).authToken;
    ...
    private fun injectDownloadCapture() {
        webView?.evaluateJavascript(
            """
            (function() {
                if (window.__kitabucodeDownload) return;
                ...
                function token() { return (window.__kitabucode || {}).authToken; }
     ...
     * either: it is read out of `window.__kitabucode` by this script, on that same
     ...
     *    which now refuses this app's own `kitabucode://callback`.
    ...
    private fun injectBridgeRelay() {
        webView?.evaluateJavascript(
            ...
            (function() {
                ...
                if (window.__kitabucodeRelayActive) return;
                window.__kitabucodeRelayActive = true;
                var ch = new BroadcastChannel('kitabucode-bridge');
                ...
                window.__kitabucodeBridgeReply = function(id, ok, payload) {
                    ch.postMessage(ok
                ...
                ch.onmessage = function(e) {
                    var d = e.data;
                    var token = (window.__kitabucode || {}).authToken;
                    ...
                    try {
                        ...
                        if (d.cmd === 'openFolderPicker') {
                            ...
                            ch.postMessage({
                                id: d.id, ok: false,
                                error: 'KitabuCode does not know the command ' + d.cmd
                            });
                        }
                    } catch(err) {
     ...
     *
     * `onTrimMemory` fires `window.__kitabucode?.onLowMemory?.(level)` and nothing
    ...
    private fun injectMemoryPressureHandler() {
        webView?.evaluateJavascript(
            """
            (function() {
                if (window.__kitabucodeMemoryHandlerActive) return;
                window.__kitabucodeMemoryHandlerActive = true;
                window.__kitabucode = window.__kitabucode || {};
                window.__kitabucode.onLowMemory = function(level) {
                    console.warn('[KitabuCode] Memory pressure: level=' + level);
                };
            })();
            """.trimIndent(),
            null
    ...
    fun showAboutDialog() {
        ...
        AlertDialog.Builder(this)
            ...
            .setNegativeButton(getString(R.string.about_privacy_policy)) { _, _ ->
                startActivity(Intent(Intent.ACTION_VIEW, "https://rmyndharis.github.io/KitabuCode/privacy-policy.html".toUri()))
            }
            .show()
    }
    ...
    private fun showLicensesDialog() {
        AlertDialog.Builder(this)
            ...
            .setNeutralButton(getString(R.string.licenses_source_code)) { _, _ ->
                startActivity(Intent(Intent.ACTION_VIEW, "https://github.com/rmyndharis/KitabuCode".toUri()))
            }
            .show()
    }
     ...
     * (kitabucode://callback?data=ENCODED_JSON) and injects it into the WebView's
        ...
        webView?.evaluateJavascript("""
            (function() {
                try {
                    ...
                    console.error('[KitabuCode] Callback relay error:', e);
                }
            })();
        """.trimIndent(), null)
    }
        ...
        AlertDialog.Builder(this)
            ...
            .setNeutralButton(getString(R.string.crash_copy_report)) { _, _ ->
                lifecycleScope.launch {
                    ...
                    val clip = android.content.ClipData.newPlainText("KitabuCode Bug Report", report)
     ...
     * (`KitabuCode: Manage Device Folder Storage`, contributed by the bundled
     ...
     *
     * It then named `KitabuCode: Clear Caches`, which exists but cannot free the
    ...
    companion object {
        ...
        private const val WORKSPACE_PREFS = "kitabucode"
         ...
         */
        private const val RELOAD_URL = "kitabucode://reload-editor"
         ...
         * KitabuCode. That refusal is also the one `force` cannot bypass, so
 ...
 * (`kitabucode://` with any host, or any scheme pointed at `callback`), the relay
...
internal fun isExtensionCallback(scheme: String?, host: String?): Boolean =
    scheme == "kitabucode" && host == "callback"
 ...
 * [KitabuCodeWebViewClient.isLocalhost] uses, because the port is what ties a
 ...
 * up in [com.kitabucode.bridge.AuthTabWindow] and gets the reading of the launch
 ...
 * the log statement redacts, and [com.kitabucode.webview.redactToken] keys on
 ...
 * resource root, and [com.kitabucode.webview.resourceRootsInForce] matches a root
    ...
    (function() {
        try {
            ...
            req.onerror = function() {
                console.warn('[KitabuCode] IndexedDB broken, reloading');
                window.location.reload();
            };
            req.onsuccess = function() { req.result.close(); };
        } catch(e) {
            console.warn('[KitabuCode] IndexedDB exception, reloading');
            window.location.reload();

package com.kitabucode.storage
...
/**
 * Manages Storage Access Framework (SAF) interactions for KitabuCode.
 *
 * Responsibilities:
...
class SafStorageManager(context: Context) {
     ...
     * ([com.kitabucode.SplashActivity]) then starts [reclaimRevokedMirrors], whose
     ...
     * (through [com.kitabucode.util.Environment]) and the preferences file, all
     ...
     * [com.kitabucode.SplashActivity], which runs on the main thread before anything is
     ...
     * scratch space: [com.kitabucode.setup.FirstRunSetup] exports it into every terminal
    ...
    internal fun reclaimRevokedMirrorsSync(): Int {
        ...
        val root = File(com.kitabucode.util.Environment.getSafMirrorsDir(context))
        var removed = 0
     ...
     *
     * The delete is [com.kitabucode.util.StorageManager.deleteRecursive] rather than
    ...
    fun getMirrorDir(safUri: Uri): File {
        return File(com.kitabucode.util.Environment.getSafMirrorDir(context, safUri))
    }
    ...
    companion object {
        private const val PREFS_NAME = "kitabucode_saf"
         ...
         *
         * [com.kitabucode.util.Environment.getSafMirrorDir] names a mirror after the first

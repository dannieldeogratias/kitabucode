...
class SafSyncEngine(private val context: Context) {
     ...
     * copy is inside KitabuCode"). What tells the user is stronger than a toast anyway:
    ...
    private fun uploadMirrorOnlyDocuments(
        ...
        // refuses them all, and forty toasts of "the only copy is inside KitabuCode",
     ...
     * only copy is inside KitabuCode" would send the user looking after a file that is
     ...
     * as a write and is the one occasion on which "the only copy is inside KitabuCode"
    ...
    companion object {
         ...
         */
        internal const val PARTIAL_SUFFIX = ".kitabucode-partial"
         ...
         * another.
         *
         * Both wrote the same scratch path, `<name>.kitabucode-partial`, so one writer's
         ...
         */
        internal const val RECORD_HEADER = "#kitabucode-saf-sync 2"
        ...
        internal fun uploadPlan(
            ...
            while (pending.isNotEmpty() && found.size <= limit) {
                ...
                for (child in children.sortedBy { it.name }) {
                    ...
                    // copy was still `.kitabucode-partial`, put that scratch file on

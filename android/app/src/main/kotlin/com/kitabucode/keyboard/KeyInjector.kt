package com.kitabucode.keyboard
...
class KeyInjector(
    ...
    fun setupModifierInterceptor() {
        ...
        val js = """
            (function() {
                if (window.__kitabucode_modifier_interceptor) return;
                window.__kitabucode_modifier_interceptor = true;
                window.__kitabucode = window.__kitabucode || {};
                window.__kitabucode.ctrl = false;
                window.__kitabucode.alt = false;
                window.__kitabucode.shift = false;
                ...
                function hookComposition(el) {
                    ...
                    if (!ec || ec.__kitabucode_hooked) return;
                    ec.__kitabucode_hooked = true;
    ...
    fun setModifierState(ctrl: Boolean, alt: Boolean, shift: Boolean) {
        webView.evaluateJavascript(
            "window.__kitabucode&&(window.__kitabucode.ctrl=$ctrl,window.__kitabucode.alt=$alt,window.__kitabucode.shift=$shift);",
            null
        )
    }
    ...
    fun queryModifierState(callback: (ctrl: Boolean, alt: Boolean, shift: Boolean) -> Unit) {
        webView.evaluateJavascript(
            "(function(){var m=window.__kitabucode||{};return JSON.stringify({c:!!m.ctrl,a:!!m.alt,s:!!m.shift})})()"
        ) { result ->
            try {

...
import com.kitabucode.R
import com.kitabucode.isExtensionCallback
import com.kitabucode.setup.ToolchainManager
import com.kitabucode.setup.ToolchainRegistry
...
import com.kitabucode.webview.urlLogLabel
 ...
 * relays on to `kitabucode://callback` is that same parameter read back out of its
 ...
 * refuses a caller without the session token. `KitabuCodeWebViewClient`'s
 ...
 * app's own `kitabucode://callback` in [AndroidBridge.openExternalUrl], which
 ...
 * which an unsolicited `kitabucode://callback` would be answered exactly as much
 ...
 *
 * The `kitabucode://callback` relay has nothing else to test. Its VIEW filter is
...
class AndroidBridge(
     ...
     * an omission. KitabuCode is a development environment: a link can point at a
     ...
     * `KitabuCodeWebViewClient.shouldOverrideUrlLoading`, which filtered nothing.
     * Same click, same URL, two outcomes, one of them silent. Both routes now
     * refuse exactly one address, this app's own `kitabucode://callback`, for the
    ...
    fun openExternalUrl(url: String, authToken: String): String {
        ...
        return try {
            ...
            //
            // `kitabucode://callback` is this app's own sign-in relay. Its VIEW
            ...
            // `kitabucode://callback`: a sign-in page on plain http, the shape this
    ...
    fun getDeviceInfo(authToken: String): String {
        ...
        return JSONObject().apply {
            ...
            put("kitabucode_version", getVersionName())
    ...
    private fun showToolchainSettings() {
        val intent = Intent(context, com.kitabucode.ToolchainActivity::class.java)
    ...
    fun generateSshKey(authToken: String, comment: String): String {
        ...
        try {
            ...
            val keyComment = if (comment.isBlank()) "kitabucode@android" else comment
     ...
     * identifier; the generated default, `kitabucode@android`, is the only one that

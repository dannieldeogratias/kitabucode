package com.kitabucode.setup

import android.content.Context
import android.system.Os
import com.kitabucode.BuildConfig
...
class FirstRunSetup(
    ...
    private val prefs = context.getSharedPreferences("kitabucode_setup", Context.MODE_PRIVATE)
    ...
    private fun installedBundledExtensionBytes(): Long {
        val extensionsDir = File(context.filesDir, "home/.kitabucode/extensions")
        val bundled = try {
    ...
    private fun createDirectories() {
        val dirs = listOf(
            ...
            "home/.ssh",
            "home/.kitabucode",
            "home/.kitabucode/extensions",
            "home/.kitabucode/data/logs",
            "home/.kitabucode/logs",
            "server",
            "usr/bin",
            "usr/lib",
            "usr/lib/git-core",
...
[ -f "${'$'}HOME/.kitabucode/toolchain-env.sh" ] && . "${'$'}HOME/.kitabucode/toolchain-env.sh"
"""
...
"""
    ...
    fun ensurePromptFix() {
        ...
        // the write stopped: an unclosed `__kitabucode_prompt()` body, no
     ...
     */
    private val npmBlockMarker = "__kitabucode_symlink_note()"

    private fun npmBashFunctions(): String = """

# npm/npx: shell functions (SELinux blocks exec of scripts under filesDir)
# KITABUCODE_PLATFORM_FIX=1: override process.platform to "linux" for npm only
...
npm() {
    KITABUCODE_PLATFORM_FIX=1 node "${'$'}PREFIX/lib/node_modules/npm/bin/npm-cli.js" --prefer-offline "${'$'}@"
    ...
    local __npm_status=${'$'}?
    [ ${'$'}__npm_status -eq 0 ] || __kitabucode_symlink_note
    return ${'$'}__npm_status
}
npx() { KITABUCODE_PLATFORM_FIX=1 node "${'$'}PREFIX/lib/node_modules/npm/bin/npx-cli.js" "${'$'}@"; }
...
__kitabucode_symlink_note() {
    [ -n "${'$'}{__KITABUCODE_SYMLINK_NOTED-}" ] && return 0
    # A directory nobody may write refuses the probe for its own reason, and a
    # note blaming symlinks for that would be wrong.
    [ -w . ] || return 0
    local probe=".kitabucode-symlink-probe.${'$'}${'$'}"
    if ln -s . "${'$'}probe" 2>/dev/null; then
        rm -f "${'$'}probe" 2>/dev/null
        return 0
    fi
    __KITABUCODE_SYMLINK_NOTED=1
    echo "kitabucode: this folder is on shared storage, which cannot hold symbolic links," >&2
    echo "kitabucode: so npm cannot create node_modules/.bin here. Move the project to" >&2
    echo "kitabucode: internal storage and open it there:  mv \"${'$'}PWD\" ~/" >&2
}
"""
...
claude() {
    ...
    # name lowercased.
    for c in "${'$'}HOME"/.kitabucode/extensions/[Aa]nthropic.claude-code-*/resources/native-binary/claude; do
        [ -f "${'$'}c" ] || continue
...
"""
    ...
    private fun migrateSettingsToMachinePath() {
        val legacy = File(context.filesDir, "home/.kitabucode/User/settings.json")
    ...
    private fun extractBundledExtensions() {
        ...
        for (name in toExtract) {
            ...
            if (!extractAssetDir("extensions/$name", "home/.kitabucode/extensions/$name", progress)) {
 ...
 */
private const val BASHRC_HEADER = "# KitabuCode bash configuration"
 ...
 */
private const val BASH_ENV_HEADER = """# KitabuCode: sourced by NON-INTERACTIVE bash through BASH_ENV.
 ...
 * working in. The v1 shape read `~/.kitabucode_folder`, which was written only
...
private const val STARTUP_DIR_BEGIN = "# >>> kitabucode startup dir"
private const val STARTUP_DIR_END = "# <<< kitabucode startup dir"
...
private val LEGACY_STARTUP_DIR_BLOCK_V1 = """
    ...
    if [ "${'$'}PWD" -ef "${'$'}HOME" ]; then
        if [ -f "${'$'}HOME/.kitabucode_folder" ]; then
            __folder="${'$'}(cat "${'$'}HOME/.kitabucode_folder" 2>/dev/null)"
...
private const val PROMPT_BEGIN = "# >>> kitabucode prompt"
private const val PROMPT_END = "# <<< kitabucode prompt"
...
private val PROMPT_BLOCK = """
    ...
    __kitabucode_prompt() {
        ...
        __kitabucode_dir="${'$'}dir"
    }
    PROMPT_COMMAND=__kitabucode_prompt
    PS1='\[\033[32m\]${'$'}{__kitabucode_dir}\[\033[0m\] \${'$'} '
 ...
 */
private const val PROMPT_ANCHOR_START = "__kitabucode_prompt() {"
 ...
 *
 * `kitabucode.*` never appears on the marketplace, so such a directory can only
 ...
 */
internal const val OWN_EXTENSION_PREFIX = "kitabucode."
 ...
 * listing it by hand.
 */
internal const val UNPACK_MARKER_NAME = ".kitabucode-unpacking"
 ...
 * `kitabucode.kitabucode-serve-network` -- new in v1.1.0 -- reached no one

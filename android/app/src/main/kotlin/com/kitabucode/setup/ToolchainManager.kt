...
class ToolchainManager(private val context: Context) {
    ...
    private val stateFile = File(context.filesDir, "home/.kitabucode/toolchains.json")
    private val envFile = File(context.filesDir, "home/.kitabucode/toolchain-env.sh")
    ...
    companion object {
         ...
         * transfer on mobile data. [com.kitabucode.util.StorageManager.clearCaches]
    ...
    private fun <T> withRedirects(
        ...
        while (redirects < MAX_REDIRECTS) {
            ...
            try {
                ...
                conn.setRequestProperty("User-Agent", "KitabuCode")
     ...
     *
     * `~/.kitabucode/toolchain-env.sh` is sourced by `.bashrc` and by the
     * `BASH_ENV` file, so a terminal and a `bash -c` pick up the toolchain's
     * environment and its loader wrappers. `~/.kitabucode/toolchain-exec.tsv`

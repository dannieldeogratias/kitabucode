...
import com.kitabucode.util.ServerLog
...
class ProcessManager(private val context: Context) {
    ...
    fun startServer(): Boolean {
        ...
        // read (KitabuCodeWebViewClient.shouldInterceptRequest and its isLocalhost).
     ...
     * records, so every KitabuCode build at that pin answers identically and the
 ...
 */
internal const val HEAP_SETTING_KEY = "kitabucode.server.heapCeilingMb"
...
internal const val HEAP_PREFS_NAME = "kitabucode"
 ...
 * from `oom_score_adj` and does not read V8 flags, so with KitabuCode in the
 ...
 * commented-out `// "kitabucode.server.heapCeilingMb": 8192` sitting in the user's

...
object ServerReadyHelper {
    ...
    // FirstRunSetup uses "kitabucode_setup" for setup_version
    private const val SETUP_PREFS = "kitabucode_setup"
    // SplashActivity uses "kitabucode" for toolchain_picker_shown
    private const val APP_PREFS = "kitabucode"
    ...
    /**
     * Pre-populates SharedPreferences so [com.kitabucode.SplashActivity] considers
     * first-run setup already done and jumps straight to [com.kitabucode.MainActivity].
     *
     * Writes to both prefs files to match how the production code uses them:
     * - `kitabucode_setup` for `setup_version` and `setup_version_code`
     *   (both read by [com.kitabucode.setup.FirstRunSetup])
     * - `kitabucode` for `toolchain_picker_shown` (read by [com.kitabucode.SplashActivity])
     *

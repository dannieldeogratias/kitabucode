...
class SettingsPathsTest {
    ...
    private val shell = "/data/user/0/com.kitabucode/files/usr/bin/bash"
    private val git = "/data/app/~~new==/com.kitabucode-new==/lib/arm64/libgit.so"

    private val oldDir = "/data/app/~~old==/com.kitabucode-old==/lib/arm64"

    /** What the Claude Code extension is launched with; a filesDir symlink. */
    private val wrapper = "/data/user/0/com.kitabucode/files/usr/bin/node"
    ...
    inner class ClaudeWrapper {
        ...
        fun `leaves a wrapper the user pointed somewhere of their own`() {
            ...
            val chosen = "/data/user/0/com.kitabucode/files/home/my-claude-wrapper.sh"
        ...
        fun `re-points a wrapper the user has stale`() {
            val stale = settings(shell, git, args = "[]",
                claudeWrapper = "/data/user/0/com.kitabucode/files/usr/bin/node-old")
            val result = requireNotNull(refreshManagedPaths(stale, shell, git, wrapper))

...
class BashrcAtomicityTest {
    ...
    private val currentMarker = ">>> kitabucode prompt"
    ...
    fun setUp() {
        ...
        every { context.applicationInfo } returns ApplicationInfo().apply {
            nativeLibraryDir = "/data/app/~~hash==/com.kitabucode-hash==/lib/arm64"
        }
    ...
    fun `an install that already has the old wrappers still gets the current block`() {
        ...
        val written = bashrc.readText()
        assertTrue(
            written.contains("__kitabucode_symlink_note()"),
    ...
    fun `a bashrc cut off after our header is cleared and written again`() {
        ...
        bashrc.writeText("# KitabuCode bash configuration\n# >>> kitabucode prompt")
    ...
    fun `a plausible-looking partial file is left alone and not guessed at`() {
        ...
        val partial = "# KitabuCode bash configuration\nexport PROJECTS_DIR='/data/projects'\n"

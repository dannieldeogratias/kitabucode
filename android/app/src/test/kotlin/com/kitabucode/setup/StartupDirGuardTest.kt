...
class StartupDirGuardTest {
     ...
     *
     * `~/.kitabucode_folder` was written only when a device folder was opened
    ...
    fun `a folder record left by an older release is ignored`() {
        ...
        createBashrc()
        File(home, ".kitabucode_folder").writeText(recorded.path)
    ...
    fun `the unguarded block is rewritten and everything around it survives`() {
        ...
        assertTrue(after.contains("kitabucode startup dir"), "the block was not replaced")
        assertFalse(
    ...
    fun `a device on the v1 block is moved to v2`() {
        ...
        assertFalse(
            after.contains(".kitabucode_folder"),
    ...
    fun `a block the user edited is left alone`() {
        ...
        assertFalse(bashrc.readText().contains("kitabucode startup dir"), "a marker was added anyway")
    }
    ...
    fun `a fresh install and a migrated one write the same block`() {
        createBashrc()
        val fresh = bashrc.readText().substringAfter("# >>> kitabucode startup dir")

        bashrc.delete()
        writeLegacyBashrc()
        FirstRunSetup(context).ensureStartupDirGuard()
        val migrated = bashrc.readText().substringAfter("# >>> kitabucode startup dir")

        assertEquals(
            fresh.substringBefore("# <<< kitabucode startup dir"),
            migrated.substringBefore("# <<< kitabucode startup dir"),

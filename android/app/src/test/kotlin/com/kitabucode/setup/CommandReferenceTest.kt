 ...
 * `KitabuCode: Something` must match a `title` contributed by a bundled extension. It is the
...
class CommandReferenceTest {
    ...
    /** Every command title contributed by a bundled extension, e.g. "KitabuCode: About". */
    ...
    fun `every command named in user-facing text is contributed by a bundled extension`() {
        ...
        for (file in sources) {
            for (m in REFERENCE.findAll(file.readText())) {
                val named = "KitabuCode: " + m.groupValues[1].trim()
        ...
        // escapes it as \"KitabuCode: ...\" while JavaScript writes it plainly. A
        ...
        assertTrue(
            ...
            "no command reference matched in any string resource, so the escaped form " +
                "\\\"KitabuCode: ...\\\" is no longer being recognised and this test is " +
    ...
    private companion object {
        /** `"title": "KitabuCode: About"` in an extension manifest. */
        val TITLE = Regex(""""title"\s*:\s*"(KitabuCode: [^"]+)"""")

        /**
         * A reference in source text: the name must be QUOTED, which is how it appears
         * when it is being given to a user to type: `\"KitabuCode: Clear Caches\"` in a
         * Kotlin literal, `"KitabuCode: Copy SSH Public Key"` inside a JavaScript one.
         *
         ...
         * injected stylesheets -- banner lines beginning "KitabuCode: Safe area padding"
         * and "KitabuCode: Enlarged touch targets" -- which name no command and never
         * reach a user.
         */
        val REFERENCE = Regex("""\\?"KitabuCode: ([A-Z][^"\\\n]*)\\?"""")
    }
}

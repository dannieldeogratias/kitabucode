...
class SupersededExtensionsTest {
    ...
    fun `handles an id that itself contains a dash`() {
        val present = bundled + "kitabucode.kitabucode-welcome-0.9.0"
        assertEquals(listOf("kitabucode.kitabucode-welcome-0.9.0"),
            supersededExtensionDirs(present, bundled))
    }
    ...
    // by the publisher filter: nothing outside kitabucode.* may ever be named.

    @Test
    fun `retires an own extension that is no longer bundled`() {
        val present = bundled + "kitabucode.kitabucode-github-auth-1.0.0"
        assertEquals(listOf("kitabucode.kitabucode-github-auth-1.0.0"),
    ...
    fun `never retires marketplace extensions or bundled own ones`() {
        val present = bundled + listOf(
            ...
            "kitabucode.kitabucode-welcome-0.9.0", // old version: superseded's job
            "extensions.json",
        )

...
class BundledExtractionSplitTest {
    ...
    private val ourMonitor = "kitabucode.kitabucode-process-monitor-1.0.0"
    private val ourWelcome = "kitabucode.kitabucode-welcome-1.2.0"
    ...
    fun `the publisher decides, not the shape of the name`() {
        ...
        val ours = "kitabucode.material-icon-theme-5.37.0"
    ...
    fun `a publisher that merely starts the same way is not ours`() {
        // `kitabucode.` includes the dot for a reason: a marketplace publisher
        // called `kitabucodetools` would otherwise be re-copied every upgrade,
        // taking its generated state with it.
        val lookalike = "kitabucodetools.helper-1.0.0"
        assertTrue(
    ...
    fun `a version bump is a supersession, not a retirement`() {
        val old = "kitabucode.kitabucode-welcome-1.2.0"
        val new = "kitabucode.kitabucode-welcome-1.2.1"
    ...
    fun `an id this build stopped shipping is retired, and only then`() {
        ...
        val dropped = "kitabucode.kitabucode-github-auth-1.0.0"
    ...
    fun `a removal names one id and leaves every other bundled extension alone`() {
        ...
        assertEquals(
            ...
            bundledDirsToExtract(
                ...
                uninstalled = setOf("ms-python.python", "kitabucode.kitabucode-process-monitor"),
            ),
        )
    }
    ...
    fun `a reinstalled extension survives once the sweep has run`() {
        ...
        val present = listOf(reinstalled, "kitabucode.kitabucode-welcome-1.2.1")

...
import com.kitabucode.util.PortFinder
...
class ProcessManagerTest {
    ...
    fun `a ceiling set in settings json reaches the command line, clamped`() {
        ...
        val settings = File(tempDir, "Machine/settings.json").apply {
            ...
            writeText("{\n    \"kitabucode.server.heapCeilingMb\": 4096,\n}\n")
        }
    ...
    fun `a device that ignores the request is not charged for its kills`() {
        ...
        val settings = File(tempDir, "Machine/settings.json").apply {
            parentFile!!.mkdirs()
            writeText("{\n    \"kitabucode.server.heapCeilingMb\": 1024,\n}\n")
        }
        every { Environment.getMachineSettingsPath(any()) } returns settings.path
    ...
    fun `a ceiling the user lowered is never suspended by the latch`() {
        ...
        val settings = File(tempDir, "Machine/settings.json").apply {
            parentFile!!.mkdirs()
            writeText("{\n    \"kitabucode.server.heapCeilingMb\": 384,\n}\n")
        }
        every { Environment.getMachineSettingsPath(any()) } returns settings.path
    ...
    fun `the start summary names what was asked for and clamps only when it clamped`() {
        ...
        val settings = File(tempDir, "Machine/settings.json").apply {
            parentFile!!.mkdirs()
            writeText("{\n    \"kitabucode.server.heapCeilingMb\": 640,\n}\n")
        }
        every { Environment.getMachineSettingsPath(any()) } returns settings.path
        assertEquals(
...
class ReadinessWithdrawalCallSiteTest {
    ...
    private val source = File("src/main/kotlin/com/kitabucode/service/ProcessManager.kt")
...
class HeapOverrideReaderTest {
    ...
    fun `a commented-out key is not honoured`() {
        ...
        val doc = """
            {
                // "kitabucode.server.heapCeilingMb": 8192,
                "extensions.verifySignature": false,
            }
        """.trimIndent()
        ...
        val doc = """
            {
                ...
                "kitabucode.server.heapCeilingMb": 1536,
            }
        """.trimIndent()
    ...
    fun `a key that is only mentioned inside another value is not honoured`() {
        ...
        val doc = """
            {
                "note": "set \"kitabucode.server.heapCeilingMb\": 4096 to tune it",
            }
        """.trimIndent()
    ...
    fun `a document written on one line is a known and accepted miss`() {
        ...
        // a comment.
        assertNull(heapOverrideFromSettings("""{"kitabucode.server.heapCeilingMb": 1024}"""))
    }
    ...
    fun `a value of the wrong type reads as absent rather than as a number`() {
        ...
        assertNull(heapOverrideFromSettings("{\n    \"kitabucode.server.heapCeilingMb\": \"1024\",\n}"))
        assertNull(heapOverrideFromSettings("{\n    \"kitabucode.server.heapCeilingMb\": true,\n}"))
        assertNull(heapOverrideFromSettings("{\n    \"kitabucode.server.heapCeilingMb\": null,\n}"))
    }

 ...
 *
 * `deviceFolderCopiesAsJson` writes JSON and the `kitabucode.manageDeviceFolders`
...
class DeviceFolderPayloadTest {
    ...
    private val producerSource = File("src/main/kotlin/com/kitabucode/MainActivity.kt")
    ...
    private fun consumerSource(): File {
        ...
        val matches = root.listFiles { entry: File ->
            entry.isDirectory && entry.name.startsWith("kitabucode.kitabucode-saf-bridge-")
        }?.toList().orEmpty()

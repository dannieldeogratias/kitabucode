...
import com.kitabucode.bridge.authRequestIdsIn
 ...
 * `kitabucode://callback` carrying BROWSABLE, so the sender is any installed app
...
class ExtensionCallbackTest {
    ...
    fun `the callback relay is recognised`() {
        assertTrue(isExtensionCallback("kitabucode", "callback"))
    }
    ...
    fun `neither half alone is enough`() {
        ...
        assertFalse(isExtensionCallback("kitabucode", "anything-else"))
    ...
    fun `a launcher intent carrying no URI is not a callback`() {
        ...
        assertFalse(isExtensionCallback("kitabucode", null))
    ...
    fun `the scheme and host are matched exactly`() {
        ...
        assertFalse(isExtensionCallback("KitabuCode", "callback"))
        assertFalse(isExtensionCallback("kitabucode", "Callback"))
        assertFalse(isExtensionCallback("kitabucode ", "callback"))
    }
}
 ...
 * window in which an unsolicited `kitabucode://callback` from anything on the
...
class AuthRequestIdTest {
    ...
    fun `the callback address names its own request`() {
        val ids = authRequestIdsIn(
            "http://127.0.0.1:13337/callback?vscode-reqid=3&vscode-scheme=kitabucode" +
    ...
    fun `an authorisation address carrying the callback in state names it too`() {
        ...
        val ids = authRequestIdsIn(
            "https://github.com/login/oauth/authorize?client_id=abc" +
                ...
                "%253Fvscode-reqid%253D7%2526vscode-scheme%253Dkitabucode" +
...
class CallbackRequestIdTest {
    ...
    fun `the id in a relay payload is read`() {
        val payload = """{"id":"7","uri":{"scheme":"kitabucode","authority":"vscode.github-authentication"}}"""
    ...
    fun `a payload with no id is refused rather than guessed at`() {
        assertNull(callbackRequestId("""{"uri":{"scheme":"kitabucode"}}"""))
...
class AuthCallbackCallSiteTest {
    ...
    private val mainActivity = File("src/main/kotlin/com/kitabucode/MainActivity.kt")
    private val bridge = File("src/main/kotlin/com/kitabucode/bridge/AndroidBridge.kt")
    ...
    fun `the relay asks whether a sign-in was in flight`() {
        ...
        assertTrue(
            calls >= 1,
            "the kitabucode://callback relay must refuse a callback no sign-in was " +
    ...
    fun `the launch arms the ids the address carries, not the fact that it launched`() {
        ...
        //
        // Not the only arming site any more. `KitabuCodeWebViewClient` arms the
...
class WorkbenchUrlTest {
    ...
    fun `the token the workbench needs is the token the redactor hides`() {
        val url = workbenchUrl(13337, "/data/data/com.kitabucode/files/home/projects", token)
...
class ResumeActionTest {
    ...
    fun `the resume decision is told about the picker, not only the sign-in`() {
        ...
        val mainActivity = File("src/main/kotlin/com/kitabucode/MainActivity.kt")

...
class TextContrastTest {
    ...
    private val pickerAdapter =
        File("src/main/kotlin/com/kitabucode/setup/ToolchainPickerAdapter.kt")
    private val extraKeyRow =
        File("src/main/kotlin/com/kitabucode/keyboard/ExtraKeyRow.kt")
    ...
    fun `a tinted button's label clears the ground it is tinted with`() {
        ...
        checkNotNull(onPrimary) {
            "Theme.KitabuCode sets no colorOnPrimary, so Material3 supplies its own from " +
    ...
    private fun themeItem(name: String): String? {
        ...
        val style = Regex("""<style\s+name="Theme\.KitabuCode".*?</style>""", RegexOption.DOT_MATCHES_ALL)
            .find(themesXml.readText())
        checkNotNull(style) { "Theme.KitabuCode is not declared in themes.xml" }
        ...
        assertTrue(
            items.size >= 5,
            "only ${items.size} items were parsed out of Theme.KitabuCode, so the " +

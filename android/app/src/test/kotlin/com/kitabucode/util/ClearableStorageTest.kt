...
class ClearableStorageTest {
    ...
    private fun directoryFor(key: String): File = when (key) {
        ...
        "extensions" -> File(filesDir, "home/.kitabucode/extensions")
        "user_data" -> File(filesDir, "home/.kitabucode/data/User")
        "logs" -> File(filesDir, "home/.kitabucode/data/logs")
     ...
     * `WebView.setDataDirectorySuffix("kitabucode")`, and on a debug install after one
    ...
    fun `the cache row leaves out the bytes the clear cannot free`() {
        val webview = File(cacheDir, "webview_kitabucode/Default/HTTP Cache/data_0").apply {
            parentFile?.mkdirs()

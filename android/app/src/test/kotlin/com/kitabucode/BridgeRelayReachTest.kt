 ...
 * the relay reads out of `window.__kitabucode` is no barrier: it lives on that same
...
class BridgeRelayReachTest {
    ...
    private val source = SourceScan.read("src/main/kotlin/com/kitabucode/MainActivity.kt")
    ...
    fun `the relay dispatcher is still what these cases are reading`() {
        ...
        assertTrue(relay.contains("BroadcastChannel('kitabucode-bridge')")) {

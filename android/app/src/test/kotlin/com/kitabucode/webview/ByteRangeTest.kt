...
class ByteRangeTest {
    ...
    fun `a closed range is taken as written`() {
        assertEquals(100L..199L, KitabuCodeWebViewClient.byteRangeOf("bytes=100-199", length))
    }

    @Test
    fun `an open range runs to the last byte`() {
        assertEquals(100L..999L, KitabuCodeWebViewClient.byteRangeOf("bytes=100-", length))
    }
    ...
    fun `a suffix range is the last n bytes`() {
        assertEquals(900L..999L, KitabuCodeWebViewClient.byteRangeOf("bytes=-100", length))
    }

    /** A player routinely asks for more than the file holds. */
    @Test
    fun `an end past the file is clamped rather than refused`() {
        assertEquals(900L..999L, KitabuCodeWebViewClient.byteRangeOf("bytes=900-99999", length))
        assertEquals(0L..999L, KitabuCodeWebViewClient.byteRangeOf("bytes=-99999", length))
    }
    ...
    fun `anything unanswerable falls back to the whole file`() {
        assertNull(KitabuCodeWebViewClient.byteRangeOf(null, length), "no header")
        assertNull(KitabuCodeWebViewClient.byteRangeOf("items=0-10", length), "another unit")
        assertNull(KitabuCodeWebViewClient.byteRangeOf("bytes=0-10,20-30", length), "multi-range")
        assertNull(KitabuCodeWebViewClient.byteRangeOf("bytes=abc-def", length), "not a number")
        assertNull(
            KitabuCodeWebViewClient.byteRangeOf("bytes=100-19x", length),
        ...
        assertNull(
            KitabuCodeWebViewClient.byteRangeOf("bytes=100", length),
        ...
        assertNull(KitabuCodeWebViewClient.byteRangeOf("bytes=1000-1100", length), "past the end")
        assertNull(KitabuCodeWebViewClient.byteRangeOf("bytes=200-100", length), "backwards")
        assertNull(KitabuCodeWebViewClient.byteRangeOf("bytes=-0", length), "an empty suffix")
        assertNull(KitabuCodeWebViewClient.byteRangeOf("bytes=0-", 0L), "an empty file")
    }

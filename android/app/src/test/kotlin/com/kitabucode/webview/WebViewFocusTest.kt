 ...
 *
 * So [KitabuCodeWebView.configure] owns two duties beyond settings: request
...
class WebViewFocusTest {
    ...
    fun `configure makes the WebView focusable in touch mode and requests focus`() {
        KitabuCodeWebView.configure(webView)
    ...
    fun `the WebView touch listener has exactly one owner`() {
        ...
        assertEquals(
            sortedMapOf("ExtraKeyButton.kt" to 1, "KitabuCodeWebView.kt" to 1),
            owners,
            "a setOnTouchListener call appeared or multiplied; a second call on " +
                ...
                "one in KitabuCodeWebView.configure instead, then update this map",
        )
    }

...
private const val FILES = "/data/data/com.kitabucode/files"
...
private val ROOTS = listOf(
    "$FILES/server",
    "$FILES/home/.kitabucode/extensions",
    "$FILES/projects",
    "$FILES/saf-mirrors",
).map(::canonical)
...
private val SENSITIVE = listOf(
    "$FILES/home/.kitabucode/data/token",
    "$FILES/home/.ssh",
).map(::canonical)
...
class WebviewResourceResolutionTest {
    ...
    fun `the connection token is not a webview resource`() {
        assertNull(resolveWebviewResource("$FILES/home/.kitabucode/data/token", ROOTS))
    }
    ...
    fun `an installed extension's own media is served`() {
        val path = "$FILES/home/.kitabucode/extensions/kitabucode.kitabucode-welcome-1.2.1/media/logo.png"
    ...
    fun `a sibling directory sharing a name prefix is refused`() {
        ...
        assertNull(resolveWebviewResource("$FILES/home/.kitabucode/extensions-old/leak.css", ROOTS))
...
class PublishedResourceRootsTest {
    ...
    fun `neither the ssh key nor the connection token falls inside a published root`() {
        ...
        assertNull(resolveWebviewResource(File(filesDir, "home/.kitabucode/data/token").path, roots))
    }
    ...
    fun `extension, server tree and workspace resources are served`() {
        ...
        assertNotNull(
            resolveWebviewResource(
                File(filesDir, "home/.kitabucode/extensions/pub.ext-1.0.0/media/main.css").path, roots
            ),
    ...
    fun `the sensitive set names the connection token and the ssh directory`() {
        ...
        assertEquals(
            listOf(
                File(filesDir, "home/.kitabucode/data/token").canonicalPath,
...
class WorkspaceRootTest {
    ...
    fun `a folder holding the connection token is refused`() {
        assertNull(workspaceRootOrNull("$FILES/home/.kitabucode/data", SENSITIVE))
    }

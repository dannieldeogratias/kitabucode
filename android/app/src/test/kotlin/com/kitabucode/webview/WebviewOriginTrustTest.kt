...
class WebviewOriginTrustTest {
    ...
    fun `the workbench's own two spellings are ours`() {
        assertTrue(KitabuCodeWebViewClient.isWorkbenchOrigin("http://127.0.0.1:$port", port))
        assertTrue(KitabuCodeWebViewClient.isWorkbenchOrigin("http://localhost:$port", port))
    }
    ...
    fun `another port on loopback is not the workbench`() {
        assertFalse(KitabuCodeWebViewClient.isWorkbenchOrigin("http://127.0.0.1:5173", port))
        assertFalse(KitabuCodeWebViewClient.isWorkbenchOrigin("https://127.0.0.1:$port", port))
    }
    ...
    fun `a webview document is ours`() {
        assertTrue(
            KitabuCodeWebViewClient.isOurOrigin("https://0f7c2b1a-uuid.vscode-cdn.net", port)
        )
    }
    ...
    fun `a document at the resource authority is not ours`() {
        assertFalse(
            KitabuCodeWebViewClient.isOurOrigin(
    ...
    fun `a remote page is not ours`() {
        assertFalse(KitabuCodeWebViewClient.isOurOrigin("https://evil.example", port))
        assertFalse(
            KitabuCodeWebViewClient.isOurOrigin("https://evil.example/vscode-cdn.net", port),
            "the suffix test must read the host and not the whole string",
        )
        assertFalse(
            KitabuCodeWebViewClient.isOurOrigin("http://x.vscode-cdn.net", port),
    ...
    fun `a parent origin is read out of the encoded query`() {
        assertEquals(
            ...
            KitabuCodeWebViewClient.queryParameter(
                "id=1&parentOrigin=http%3A%2F%2F127.0.0.1%3A41234&swVersion=4", "parentOrigin"
            ),
        )
        assertNull(
            KitabuCodeWebViewClient.queryParameter("id=1&swVersion=4", "parentOrigin"),
            "absent is the ordinary case: every asset request carries no parent origin",
        )
        assertNull(KitabuCodeWebViewClient.queryParameter(null, "parentOrigin"))
    }
    ...
    fun `a parameter whose name only ends in the one asked for is not read`() {
        assertNull(
            KitabuCodeWebViewClient.queryParameter("xparentOrigin=https%3A%2F%2Fevil", "parentOrigin")
        )
    }
    ...
    fun `a valueless parameter is an empty value and not an absent one`() {
        assertEquals("", KitabuCodeWebViewClient.queryParameter("id=1&parentOrigin", "parentOrigin"))
    }
}

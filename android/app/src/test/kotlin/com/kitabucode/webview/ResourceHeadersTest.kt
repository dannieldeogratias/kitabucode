...
class ResourceHeadersTest {
    ...
    fun `a request with no origin is answered same-site`() {
        val headers = KitabuCodeWebViewClient.resourceHeaders(null)
    ...
    fun `the served type stays advisory`() {
        assertFalse(
            KitabuCodeWebViewClient.resourceHeaders(null).containsKey(nosniff),
            "nosniff refuses every image and audio file the type table does not name",
        )
        assertFalse(KitabuCodeWebViewClient.resourceHeaders(webview).containsKey(nosniff))
    }
    ...
    fun `an asker is echoed and still carries the policy`() {
        val headers = KitabuCodeWebViewClient.resourceHeaders(webview)
    ...
    fun `the served arm composes its headers from the helper`() {
        val source = File("src/main/kotlin/com/kitabucode/webview/KitabuCodeWebViewClient.kt").readText()
...
class RefererGateTest {
    ...
    fun `a webview document is ours`() {
        assertTrue(KitabuCodeWebViewClient.isOurReferer(webview, port))
        assertTrue(
            KitabuCodeWebViewClient.isOurReferer("$webview" + "some/path.html?q=1", port),
    ...
    fun `the workbench is ours`() {
        assertTrue(KitabuCodeWebViewClient.isOurReferer("http://127.0.0.1:$port/", port))
        assertTrue(KitabuCodeWebViewClient.isOurReferer("http://localhost:$port/?folder=%2Fx", port))
        assertFalse(
            KitabuCodeWebViewClient.isOurReferer("http://127.0.0.1:${port + 1}/", port),
    ...
    fun `the resource authority is a referer we answer`() {
        assertTrue(
            KitabuCodeWebViewClient.isOurReferer(
    ...
    fun `a foreign page is refused`() {
        // The measured attack.
        assertFalse(KitabuCodeWebViewClient.isOurReferer("http://10.0.2.2:8877/", port))
        assertFalse(KitabuCodeWebViewClient.isOurReferer("https://example.com/x.html", port))
    }
    ...
    fun `no referer at all is refused`() {
        assertFalse(
            KitabuCodeWebViewClient.isOurReferer(null, port),
    ...
    fun `the string cannot be smuggled in the path or the query`() {
        assertFalse(
            KitabuCodeWebViewClient.isOurReferer("http://10.0.2.2:8877/x.vscode-cdn.net", port),
            "the origin is what is judged, not the whole URL",
        )
        assertFalse(
            KitabuCodeWebViewClient.isOurReferer("http://10.0.2.2:8877/?a=.vscode-cdn.net", port),
        )
        assertFalse(
            KitabuCodeWebViewClient.isOurReferer("http://evil.example/#.vscode-cdn.net", port),
        )
    }
    ...
    fun `a lookalike host is refused`() {
        assertFalse(KitabuCodeWebViewClient.isOurReferer("https://evilvscode-cdn.net/", port))
        assertFalse(KitabuCodeWebViewClient.isOurReferer("https://vscode-cdn.net.evil.com/", port))
        assertFalse(
            KitabuCodeWebViewClient.isOurReferer("http://x.vscode-cdn.net/", port),
    ...
    fun `something that is not an absolute URL is refused`() {
        assertFalse(KitabuCodeWebViewClient.isOurReferer("", port))
        assertFalse(KitabuCodeWebViewClient.isOurReferer("about:blank", port))
        assertFalse(KitabuCodeWebViewClient.isOurReferer("/just/a/path", port))
        assertFalse(KitabuCodeWebViewClient.isOurReferer("data:text/html,x", port))
    }
}

...
import com.kitabucode.authCallbackIsExpected
 ...
 *
 * One address is refused, and it is not a destination: `kitabucode://callback`
...
class ExternalUrlHandoffTest {
    ...
    private lateinit var client: KitabuCodeWebViewClient
    ...
    private companion object {
        ...
        const val OWN_CALLBACK =
            "kitabucode://callback?data=%7B%22id%22%3A%22911%22%2C%22uri%22%3A%22x%22%7D" +
                "&vscode-reqid=911"
    }
     ...
     * `kitabucode://callback`, in the two steps it makes it in: a request this app
    ...
    fun setUp() {
        ...
        client = KitabuCodeWebViewClient(
     ...
     * has no component for `kitabucode://retry-server`. The exception is caught
    ...
    fun `the retry navigation is answered here, not handed to another app`() {
        ...
        val handled = client.shouldOverrideUrlLoading(
            view, request("kitabucode", "retry-server", -1, RETRY_URL)
        )
    ...
    fun `a subframe cannot restart the server`() {
        ...
        client.shouldOverrideUrlLoading(
            view, request("kitabucode", "retry-server", -1, RETRY_URL, fromMainFrame = false)
        )
    ...
    fun `a retry URL carrying anything extra is not answered here`() {
        ...
        client.shouldOverrideUrlLoading(
            view, request("kitabucode", "retry-server", -1, "$RETRY_URL?reload=1")
        )
     ...
     * `kitabucode://callback` is not a destination in another app but this app's
    ...
    fun `this app's own sign-in callback is refused rather than handed over`() {
        val handled = client.shouldOverrideUrlLoading(
            view, request("kitabucode", "callback", -1, OWN_CALLBACK)
        )
        ...
        assertNull(
            ...
            "the page chose which kitabucode://callback this app will accept, and the " +
    ...
    fun `a tapped subframe link to this app's own callback launches nothing`() {
        val handled = client.shouldOverrideUrlLoading(
            view,
            request(
                "kitabucode", "callback", -1, OWN_CALLBACK,
     ...
     * not http or https. It recorded nothing, so the `kitabucode://callback` that
     ...
     * can post a `kitabucode://callback` naming one of them, through an exported
    ...
    fun `a sign-in address in a subframe opens no callback window`() {
        ...
        assertFalse(
            callbackWouldBeTaken(SIGN_IN_REQUEST_ID),
            "a frame this app does not vouch for chose which kitabucode://callback the app " +
    ...
    fun `a link carrying no sign-in opens no callback window`() {
        ...
        assertFalse(
            ...
            "following a documentation link recorded a launch. Every external link in the " +
                ...
                "kitabucode://callback is taken in.",
        )
    }

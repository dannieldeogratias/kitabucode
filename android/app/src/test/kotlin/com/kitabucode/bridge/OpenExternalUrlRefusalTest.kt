...
class OpenExternalUrlRefusalTest {
     ...
     * For ten minutes after that, a `kitabucode://callback` naming that request
    ...
    fun `a launch that fails puts the auth callback window back`() {
        ...
        assertNull(
            ...
            "a launch that threw left the callback window armed. For the next ten minutes any " +
                "app on the device can post a kitabucode://callback naming that request and this " +
     ...
     * armed.
     *
     * `kitabucode://callback` is the address the browser fires to hand a finished
    ...
    fun `the app's own sign-in callback is refused, and arms nothing`() {
        ...
        every { callback.scheme } returns "kitabucode"
        ...
        val answer = bridge.openExternalUrl(
            "kitabucode://callback?vscode-reqid=$OWN_CALLBACK_REQUEST_ID" +
    ...
    fun `the auth window is armed before the launch, not after`() {
        val source = File("src/main/kotlin/com/kitabucode/bridge/AndroidBridge.kt")
    ...
    fun `an http address opened through the system browser arms the callback window`() {
        ...
        // Both return by the same `kitabucode://callback`, and a window armed on

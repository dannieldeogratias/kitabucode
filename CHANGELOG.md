...
- **KitabuCode: Open Recent Folder** is now on the remote indicator in the status bar. Typing its name into the Command Palette was the only way to reach the device folder picker, which is why people could not find it.
...
- The Toolchains screen has a Command Palette entry, **KitabuCode: Manage Toolchains**. The launcher icon's shortcut was the only route to it.
- **KitabuCode: About**, which carries the licence notices and the written offer of source, is now on the remote indicator in the status bar as well as in the Command Palette.
...
- A unit-test coverage report, on request: `./gradlew :app:createDebugUnitTestCoverageReport -PkitabucodeCoverage`. Nothing gates on the figure and the suite CI runs stays uninstrumented.
...
- The onboarding screen, the extension reload prompts and the profile-switch dialog name KitabuCode instead of Visual Studio Code.
- The sign-in callback page is branded as KitabuCode and no longer carries the VS Code logo.
...
- Claude Code works on Android 13 and 14 again. Its runtime calls `epoll_pwait2`, which an app may only use from Android 15, and the kernel stops the process there; KitabuCode now answers that call with `epoll_pwait` so sign-in and everyday use work on the older releases too.
...
- A page can no longer navigate the editor to the app's own `kitabucode://callback` and have it accepted as a finished sign-in. The link route now refuses that address, as the browser route already did.
...
- A file created in a device folder while the write-back could not run reaches the device the next time that folder is opened, instead of staying inside KitabuCode until the app is uninstalled.
...
- Opening a kitabucode:// link on an install that had never finished setting up started the editor with no server tree behind it; it now runs setup first and keeps the link.
...
- KitabuCode no longer offers itself in Android's "Open with" list for source files. It advertised twenty file types and opened none of them.
...
- `CONTRIBUTING.md` no longer describes a URL allow-list the app does not have. KitabuCode is a development environment and reaches LAN hosts on purpose; the session token is what is checked.
...
- KitabuCode warns when the installed Android System WebView is older than Chrome 105, the version it is tested against. It warns and continues rather than refusing to start, and a version it cannot read is not treated as an old one.
...
- The privacy policy describes Android backup, which copies `~/.kitabucode/data/Machine` to your Google account. SSH keys, tokens, projects and toolchains are excluded by an allowlist.
...
- The loopback DNS proxy requires a per-boot token. Any installed app could previously use it as an open forwarder attributed to KitabuCode, and a rejected tunnel no longer leaves a connection pinned open.
...
- Terminal profile picker was empty, leaving no way to switch terminals ([#3](https://github.com/rmyndharis/KitabuCode/issues/3)).
...
- Cold start no longer crashes while the WebView still shows its placeholder URL. Thanks [@4in4in](https://github.com/4in4in) ([#6](https://github.com/rmyndharis/KitabuCode/pull/6)).
...
- A browser launch that failed left the sign-in callback window open for ten minutes, during which any `kitabucode://callback` on the device was accepted.
...
- The web walkthrough greets users with KitabuCode branding again, and the hamburger menu returned to touch-friendly sizing.
...
- The low-storage warning names something you can do. It pointed at a Settings screen that does not exist; **KitabuCode: Show Storage Usage** and **KitabuCode: Clear Caches** are now reachable.
...
KitabuCode is now publicly available on Google Play. This release represents the cumulative work across milestones M0 to M6, bringing a full VS Code IDE experience to Android.

### Added
- CI/CD pipeline: test job in CI, tag-triggered release workflow, GitHub Pages deployment
- Privacy policy hosted on GitHub Pages
- "KitabuCode: About" command in command palette with version info and legal links
...
This release represents the cumulative work across milestones M0 through M5, bringing KitabuCode from initial project structure to a fully functional IDE on Android.
...
- product.json branding (KitabuCode, Open VSX marketplace)
...
- Android intent handling for "Open with KitabuCode"

[Unreleased]: https://github.com/rmyndharis/KitabuCode/compare/v1.2.0...HEAD
[1.2.0]: https://github.com/rmyndharis/KitabuCode/compare/v1.1.0...v1.2.0
[1.1.0]: https://github.com/rmyndharis/KitabuCode/compare/v1.0.0...v1.1.0
[1.0.0]: https://github.com/rmyndharis/KitabuCode/compare/v0.2.9...v1.0.0
[0.2.9]: https://github.com/rmyndharis/KitabuCode/compare/v0.2.8...v0.2.9
[0.2.8]: https://github.com/rmyndharis/KitabuCode/compare/v0.2.5...v0.2.8
[0.2.5]: https://github.com/rmyndharis/KitabuCode/compare/v0.2.4...v0.2.5
[0.2.4]: https://github.com/rmyndharis/KitabuCode/compare/v0.2.3...v0.2.4
[0.2.3]: https://github.com/rmyndharis/KitabuCode/compare/v0.2.2-m6...v0.2.3
[0.2.2-m6]: https://github.com/rmyndharis/KitabuCode/compare/v0.1.0-alpha...v0.2.2-m6
[0.1.0-m0]: https://github.com/rmyndharis/KitabuCode/releases/tag/v0.1.0-alpha

# Contributing to KitabuCode

Thank you for your interest in contributing to KitabuCode. This guide covers everything you need to set up a development environment, build the app, test on device, and submit changes.
...
git clone https://github.com/<your-username>/KitabuCode.git
cd KitabuCode

# Add upstream remote
git remote add upstream https://github.com/rmyndharis/KitabuCode.git
```
...
│   ├── app/
│   │   ├── src/main/
│   │   │   ├── kotlin/com/kitabucode/  # Kotlin source code
...
│   │   │   │   ├── KitabuCodeApp.kt       # Application class, WebView pre-warm
...
│   │   │   │   └── extensions/           # kitabucode.* in git, Open VSX ones downloaded
...
| `KitabuCodeApp.kt` | Application class, WebView pre-warm, CrashReporter init |
| `bridge/AndroidBridge.kt` | JS interface: clipboard, file picker, OAuth, SSH, storage, toolchains |
| `bridge/SecurityManager.kt` | The bridge session token: issues it, and validates it on every `@JavascriptInterface` call. It judges the caller, never the destination: there is no URL allow-list, and WebView navigation is decided in `KitabuCodeWebViewClient.shouldOverrideUrlLoading`. That callback is not one rule: an external main-frame navigation is handed to the system, an http or https subframe is rendered in place instead, and a subframe naming any other scheme needs a user gesture behind it or nothing leaves |
...
| `webview/KitabuCodeWebViewClient.kt` | CDN URL interception, vscode-resource serving, crash recovery |
...
| `check-patch-fingerprints.py` | Checks a packaged tree carries every patch in `patches/`, using the expectations in `patches/fingerprints.txt`. It also reads back `kitabucode-patches.json`, which `build-vscode-oss.sh` writes into the tree with `--write-manifest`, and compares the sha256 of each patch's diff against this checkout's, so a tree built from different patch text is refused rather than passing on a pattern that still matches. That verdict is reported first, before the rows. Takes the tree as an argument, so the same check can run against a downloaded tarball | exit status |
...
Output: `android/app/build/outputs/apk/debug/app-debug.apk` (debug package name: `com.kitabucode.debug`)
...
```bash
export KITABUCODE_KEYSTORE_FILE=/path/to/keystore.jks
export KITABUCODE_KEYSTORE_PASSWORD=...
export KITABUCODE_KEY_ALIAS=...
export KITABUCODE_KEY_PASSWORD=...
...
adb shell pm clear com.kitabucode.debug

# Launch via SplashActivity (required after clearing data for first-run extraction)
adb shell am start -n com.kitabucode.debug/com.kitabucode.SplashActivity
```
...
```bash
# Get the app PID
adb shell ps -A | grep kitabucode
...
It starts `com.kitabucode.debug`, which is what `assembleDebug` produces; set `PKG` to
...
adb logcat -s KitabuCode.MainActivity KitabuCode.NodeService KitabuCode.ProcessManager KitabuCode.ToolchainManager

# Everything the app process writes
adb logcat --pid=$(adb shell pidof com.kitabucode.debug)

# Memory
adb shell dumpsys meminfo com.kitabucode.debug
...
`ProcessManager`, under `KitabuCode.ProcessManager` with a `[node]` prefix. That goes through
...
```bash
adb shell run-as com.kitabucode.debug ls files/home/.kitabucode/data/logs
```
...
adb shell run-as com.kitabucode.debug cat files/server/editor-server.pid
...
In `android/app/src/main/kotlin/com/kitabucode/setup/FirstRunSetup.kt`, add the tool to `setupToolSymlinks()`:
...
3. **Leave a fingerprint.** The build's Verify stage greps the packaged bundles for a string from
   ...
   `kitabucode-patches.json`, the sha256 of every patch's diff that `build-vscode-oss.sh` writes
...
- `assets/server.js`, `assets/process-monitor.js`, `assets/platform-fix.js`, `assets/dns-proxy.js` and the four `assets/extensions/kitabucode.*` trees are hand-written JavaScript (not minified) and are the only parts of `assets/` in git. Keep them readable.
...
1. Check [existing issues](https://github.com/rmyndharis/KitabuCode/issues) and [PRs](https://github.com/rmyndharis/KitabuCode/pulls) to avoid duplicating work.
2. For large changes, open an issue first to discuss the approach.
3. Issues labeled [`good first issue`](https://github.com/rmyndharis/KitabuCode/labels/good%20first%20issue) or [`help wanted`](https://github.com/rmyndharis/KitabuCode/labels/help%20wanted) are good starting points.
...
Before reporting, check [existing issues](https://github.com/rmyndharis/KitabuCode/issues).
...
Use the [Feature Request template](https://github.com/rmyndharis/KitabuCode/issues/new?template=feature_request.md). Describe:
...
- [Discussions](https://github.com/rmyndharis/KitabuCode/discussions) -- for questions and general discussion
- [Issues](https://github.com/rmyndharis/KitabuCode/issues) -- for bugs and feature requests

---

Thank you for helping make KitabuCode better.

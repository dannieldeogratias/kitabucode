# KitabuCode User Guide

A practical guide to using KitabuCode -- the full VS Code IDE running natively on your Android device.
...
KitabuCode is VS Code. If you have used VS Code on desktop, everything works the same way.
...
- For a folder anywhere else on the device, tap the remote indicator at the left
  end of the status bar and choose **KitabuCode: Open Recent Folder**, then
...
KitabuCode stores settings in `~/.kitabucode/`. Key defaults:
...
`android/app/src/main/kotlin/com/kitabucode/keyboard/KeyPageConfig.kt`, and the
...
Open the terminal with **Ctrl+`** or from the menu bar. KitabuCode includes a full terminal with real PTY support, so full-screen and interactive programs work natively: tmux, bash line editing, and the Node and Python REPLs.
...
KitabuCode uses the [Open VSX](https://open-vsx.org) extension registry. This is a free, open alternative to the Microsoft Marketplace. Most popular extensions are available.
...
These extensions come bundled with KitabuCode:
...
KitabuCode also ships four of its own, which do not appear in the marketplace:
...
KitabuCode opens on the editor's own dark theme, and it is not the only one installed. Nineteen
...
Extensions that use webview panels (such as theme configurators, documentation viewers, and AI assistants) render correctly in KitabuCode.
...
KitabuCode creates a default SSH config at `~/.ssh/config` on first launch with sensible defaults:
...
npm and npx work as expected. A few notes specific to KitabuCode:
...
Beyond the bundled tools (Node.js, Python, Git, Bash), KitabuCode offers additional languages as on-demand downloads.
...
[latest GitHub Release](https://github.com/rmyndharis/KitabuCode/releases/latest).
...
two routes. From the editor, run **KitabuCode: Manage Toolchains** from the Command
Palette (**Ctrl+Shift+P**). From outside it, **touch and hold the KitabuCode icon** (on
...
The status bar shows a phantom process count. This tells you how many background processes KitabuCode is using.
...
- An idle language server is not killed, by a timer or by hand: its extension restarts it within a second. Disabling the extension that starts it is what frees the slot; **KitabuCode: Show Process Tree** marks the idle ones.
...
Packages that require C/C++ compilation (node-gyp) fail on KitabuCode because there is no C compiler on the device. This affects packages like `better-sqlite3`, `bcrypt`, `sharp`, `canvas`, and `node-sass`. Pure JavaScript or WASM alternatives exist for most of them (see the [Web Development](#package-compatibility) section).
...
builds and install normally, which is why KitabuCode reports the platform it
...
installed toolchains live. KitabuCode works around it by handing the file to the
...
Android 12 and later enforce a system-wide limit of 32 phantom processes (background processes spawned by apps). KitabuCode minimizes its footprint:
...
whose language servers **KitabuCode: Show Process Tree** marks idle.

### Memory Usage

KitabuCode typically uses 400-700 MB of RAM. On devices with 4 GB or less, you may experience occasional restarts under memory pressure. Tips:

- Close browser tabs and other apps to free RAM.
- Limit concurrent terminals to 1-2.
- Language servers are the biggest memory consumers. Nothing sheds them automatically, because their extensions restart them; disable the extensions you are not using. **KitabuCode: Show Process Tree** marks the idle ones.
...
KitabuCode answers that one call itself, so sign-in and everyday use work on
...
call`, a newer release of the extension is asking for a call KitabuCode does not
...
a fault in KitabuCode: a release build writes an extension's startup failure to a
...
into the editor is the softer case: KitabuCode ships a compatibility layer that
...
KitabuCode asks the marketplace for the musl build wherever an extension
...
is not available in KitabuCode for the Alpine ARM 64 platform.` Alpine is not what
your phone is running; it is the build KitabuCode asks for, for the reason above.
...
carries no such address and KitabuCode adds none, so the download URL is empty and
...
VS Code's web client runs as a single window. You cannot open multiple VS Code windows side by side. However, you can use Android's split-screen mode to pair KitabuCode with another app (like a browser for previewing).
...
> area of shared storage (`Android/data/com.kitabucode/files/projects`) keeps
...
- A debug build is readable with `adb shell run-as com.kitabucode.debug`, and its
...
- An install that still keeps its projects on shared storage can be read as
  ...
  ```
  adb pull /storage/emulated/0/Android/data/com.kitabucode/files/projects
  ```
...
- Or run **KitabuCode: Open Folder from Device** from the Command Palette
...
Then clear app data from Settings > Apps > KitabuCode > Clear Data and relaunch.
...
# Check disk usage
du -sh ~/projects/*
du -sh ~/.kitabucode/extensions/*
```

To remove an installed toolchain, run **KitabuCode: Manage Toolchains** from the
...
3. To reach the server from **another device** on the same network, restart it
   ...
   Next.js, `--bind 0.0.0.0` for Python) and run **KitabuCode: Serve on Network**
   from the Command Palette for the address the other device should use.

### WebView Crash Recovery

If the editor UI crashes but the app stays open, KitabuCode automatically recovers the WebView and reconnects to the running server. Your terminal sessions and unsaved work in the editor state are preserved.
...
*KitabuCode is built from the MIT-licensed Code - OSS source code. Not affiliated with or endorsed by Microsoft Corporation. "Visual Studio Code" and "VS Code" are trademarks of Microsoft. Uses Open VSX extension registry, not Microsoft Marketplace.*

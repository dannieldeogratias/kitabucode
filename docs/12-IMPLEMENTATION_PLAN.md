...
> the Kotlin under `android/app/src/main/kotlin/com/kitabucode/`. `CONTRIBUTING.md` is the prose
...
> than description:
>
> - **"Open with KitabuCode" file-type intent filters** (M2-T6). `AndroidManifest.xml` declares no
...
> - **A GitHub OAuth flow owned by Kotlin** (`kitabucode://oauth/github`, `startGitHubOAuth`).
>   No such method exists in the Kotlin. What ships is one opaque relay on `kitabucode://callback`,
...
This document translates the KitabuCode documentation suite (PRD, SRS, Architecture, Technical Spec, Milestones) into an actionable, week-by-week implementation plan with explicit task dependencies, file inventories, and validation checkpoints.
...
```mermaid
gantt
  title KitabuCode Implementation Timeline
...
│           ├── kotlin/com/kitabucode/
│           │   ├── KitabuCodeApp.kt     (Application class)
...
2. Configure `app/build.gradle.kts`:
   ```kotlin
   android {
       namespace = "com.kitabucode"
       compileSdk = 36
       defaultConfig {
           applicationId = "com.kitabucode"
           minSdk = 33
           targetSdk = 36
           versionCode = 1
...
4. Create `KitabuCodeApp.kt` with `WebView.setDataDirectorySuffix("kitabucode")` in `onCreate()`
...
2. **Day 2 (cross-compilation attempt)**:
   ...
   - Run configure:
     ...
     ./configure \
       ...
       --prefix=/data/data/com.kitabucode/files/usr
     ```
...
2. Build and test: `docker build -t kitabucode/build-env toolchains/`
3. Verify NDK compiler works inside container:
   ```bash
   docker run kitabucode/build-env aarch64-linux-android28-clang --version
   ```
...
```
android/app/src/main/kotlin/com/kitabucode/
├── service/
...
└── webview/
    ├── KitabuCodeWebView.kt      (WebView settings configuration)
    └── KitabuCodeWebViewClient.kt (URL loading, error handling)
```

**Implementation steps**:

1. **KitabuCodeWebView.kt** (configure WebView settings):
...
2. **KitabuCodeWebViewClient.kt** (handle page load errors):
...
patches/
├── code-server/
│   ├── product.diff                (branding → "KitabuCode", Open VSX URLs)
...
│   ├── github-auth.diff
│   └── local-storage.diff
└── kitabucode/                      (empty for M1, populated M2+)
...
2. **Day 2 (apply and customize patches)**:
   ...
   - Create `patches/code-server/product.diff` with KitabuCode branding:
     ```json
     {
       "nameShort": "KitabuCode",
       "nameLong": "KitabuCode",
       "applicationName": "kitabucode",
       "dataFolderName": ".kitabucode",
       "extensionsGallery": {
...
1. **FirstRunSetup.kt** (extract VS Code assets):
   ...
   - Create required directories: `home/`, `home/.kitabucode/`, `home/.kitabucode/extensions/`
   - Track extraction version for future upgrades
   - Configure VS Code settings (`settings.json`):
     ```json
     {
       "terminal.integrated.profiles.linux": {
         "bash": { "path": "/data/data/com.kitabucode/files/usr/bin/bash" }
       },
       "terminal.integrated.defaultProfile.linux": "bash",
       "git.path": "/data/data/com.kitabucode/files/usr/bin/git"
     }
     ```
...
3. **ProcessManager.kt** (update server launch):
   ...
   val command = listOf(
       ...
       "--host=127.0.0.1",
       "--port=$port",
       "--extensions-dir=$home/.kitabucode/extensions",
       "--user-data-dir=$home/.kitabucode",
       "--server-data-dir=$home/.kitabucode",
       "--log=info"
   )
   ```
...
4. **Day 4** (test and verify phantom process count):
   ...
   - `adb shell ps -A | grep kitabucode` → verify only 1 tmux process
...
| 10  | Phantom processes ≤ 5                                    | `adb shell ps -A \| grep kitabucode`          |
...
└── webview/
    └── KitabuCodeWebView.kt      (viewport handling)
...
2. **ClipboardBridge.kt**:
   ...
   - Test: copy in Chrome → paste in KitabuCode and vice versa
...
- [ ] Copy/paste works between KitabuCode and other apps
...
└── kotlin/com/kitabucode/
    ...
    └── webview/
        └── KitabuCodeWebViewClient.kt  (onRenderProcessGone)

patches/kitabucode/
└── android-intent.diff             ("Open with KitabuCode")
```
...
- [ ] ~~"Open with KitabuCode" appears in Android share/open menus for code files~~ **Withdrawn**, and
...
| 9   | ~~"Open with KitabuCode" works~~ **withdrawn, see M2-T6 note** | n/a: the app registers no file filter |
...
└── extensions/
    ...
    ├── kitabucode.kitabucode-welcome-1.0.0/
    ├── kitabucode.kitabucode-saf-bridge-1.0.0/
    └── kitabucode.kitabucode-process-monitor-1.2.0/
```
...
2. **Day 2** (develop 3 custom KitabuCode extensions):
   - `kitabucode-welcome`: Welcome tab with quick actions ("Open Folder", "Clone Repo", "New File", tool versions)
   - `kitabucode-saf-bridge`: VS Code UI for SAF folder management (used in M4)
   - `kitabucode-process-monitor`: Phantom process monitoring UI (used in M4)
...
1. **CrashReporter.kt**:
   - Initialize in `KitabuCodeApp.onCreate()` (app-level, catches all crashes)
...
2. **Logger.kt**:
   ...
   - All KitabuCode components use Logger instead of raw `Log.d()`
...
```
# Note: the KitabuCode patches (extension host, ptyHost, and the rest) are
...
4. Integrate with `kitabucode-process-monitor` extension (from M3)
...
- [ ] `adb shell ps -A | grep kitabucode` confirms count
...
2. Handle deep link callback: `kitabucode://oauth/github?code=...&state=...`
3. Forward OAuth token to WebView via `window.kitabucode.onOAuthCallback()`
...
3. **Day 5** (integration):
   - `kitabucode-saf-bridge` extension (from M3) provides UI
...
> [!NOTE]
> **Package Manager** (`kitabucode pkg`) was originally scoped for M5 but has been deferred to the **Post-Release Roadmap**. Similarly, **Rust** and **C/C++ (Clang)** toolchains are deferred to Post-Release; M5 focuses on Go, Ruby, and Java only. Toolchain compatibility verification and APK size audit have been moved to M6 (Release) where they logically belong as verification/release activities.
...
> `scripts/build-vscode-oss.sh`. There is no `patches/kitabucode/` directory.
...
3. **Day 4**: the Toolchains screen (`ToolchainActivity`). Planned as a Settings page; what shipped
   ...
   - Opened from the launcher shortcut `SplashActivity.publishToolchainShortcut()` pushes, or from the Command Palette: the bundled SAF bridge extension registers **KitabuCode: Manage Toolchains**, which sends `openToolchainSettings` over the relay to `AndroidBridge`
...
1. **Extension OAuth callback relay**: Chrome Custom Tabs → Android Intent → WebView (`kitabucode://callback`)
...
1. Design KitabuCode icon/logo (original, not VS Code's)
...
- [ ] Original KitabuCode icon designed (not VS Code's blue icon)
...
7. Secrets management: keystore env vars (`KITABUCODE_KEYSTORE_*`), `LIBNODE_URL`
...
| **VS Code patches**        | ~19 files    | 9 code-server patches + 10 KitabuCode patches (incl. ext-host-worker, pty-host-worker, android-fs)                                                                                                                            |
...
| **Custom extensions**      | 3 dirs       | kitabucode-welcome, kitabucode-saf-bridge, kitabucode-process-monitor                                                                                                                                                           |

...
KitabuCode has three interface boundaries:
...
one channel, and the relay reads the token out of `window.__kitabucode` on the same origin,
...
1. **Per-session token**: `SecurityManager` generates 32 random bytes at construction
   ...
   `MainActivity.injectBridgeToken()` writes it to `window.__kitabucode.authToken`
...
There is no `window.__kitabucode.injectKey`, and this block described one until it
...
window.__kitabucode.onLowMemory(level: number); // Android trim-memory level
```
...
`window.__kitabucode.authToken`, which MainActivity sets once the bridge is installed.
...
saf-bridge extension contributes **KitabuCode: Manage Toolchains**, so the Toolchains
...
`KitabuCode does not know the command <name>`. Before that, the caller's promise died on
...
// judged before anything else happens: this app's own `kitabucode://callback`
...
// navigation route in `KitabuCodeWebViewClient.shouldOverrideUrlLoading` refuses
...
// still opening, or it was open in this session and KitabuCode must be restarted.
...
sequenceDiagram
  ...
  C-->>K: VIEW intent, kitabucode://callback?data=ENCODED_JSON
...
The scheme is `kitabucode://callback`, not `kitabucode://oauth/<provider>`, and the
...
|---|---|---|
| `isExtensionCallback(scheme, host)` | `MainActivity.kt` | Anything that is not exactly scheme `kitabucode` **and** host `callback` |
...
// "KitabuCode: Manage Toolchains". The launcher long-press shortcut that
...
This is VS Code's built-in protocol. KitabuCode uses it as-is (no modifications needed).
...
spent the app stops trying and says so: the notification switches to "KitabuCode server
...
Install:
  ...
  2. Extract to ~/.kitabucode/extensions/{publisher}.{name}-{version}/
...
**2. `assets/extensions/`**: nine directories, extracted to `~/.kitabucode/extensions/`
on first run:
...
flowchart TD
  ...
  O --> O1["kitabucode.kitabucode-saf-bridge (the 10 KitabuCode: commands)"]
  O --> O2["kitabucode.kitabucode-welcome (Get Started walkthrough)"]
  O --> O3["kitabucode.kitabucode-process-monitor"]
  O --> O4["kitabucode.kitabucode-serve-network (dev-server preview)"]
```
...
`kitabucode.kitabucode-*/`, because this project's own extensions are source and the rest
...
array plus the four `kitabucode.*` directories; do not enumerate this set from git.
...
> ⚠️ **Proposed. This CLI does not exist.** There is no `kitabucode` executable and no
...
> occurrence of `kitabucode pkg` in this repository is in a document.
>
...
> only control (`kitabucode://reload-editor`) loads the editor again and clears the record.

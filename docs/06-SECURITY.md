...
| App-private storage                  | Files in /data/data/com.kitabucode/, inaccessible to other apps. The projects tree is the one thing not there, see 5.2                                                                                                              |
...
| No dynamic code loading (Play Store) | Core binaries bundled as .so in APK; toolchains as on-demand asset packs via Play Store. No runtime binary download in Play Store version. Sideload installs download toolchain ZIPs from GitHub Releases; `kitabucode pkg` is planned, not shipped |
...
| App-private storage | Toolchains extracted to /data/data/com.kitabucode/, inaccessible to other apps  |
...
| -------------------------------------- | ---------------------------------------------- |
| Machine-scoped defaults (`home/.kitabucode/data/Machine`) | Included. It is the only path the allowlist names |
...
enforced in `KitabuCodeWebViewClient.shouldInterceptRequest` because nothing else is in a position
...
- **The web extension host is same-origin with the workbench.** `product.json` carries no
  ...
  `window.__kitabucode` on that same origin. `MainActivity.injectBridgeRelay` carries the per-command
...
| **Telemetry**   | None from KitabuCode or the editor build | Nothing is collected or transmitted by this app. The bundled chat extension carries senders of its own, gated on the editor's telemetry level; see 3.1 |
...
| Tree                                                                | Where                                                                        |
| ------------------------------------------------------------------- | ---------------------------------------------------------------------------- |
| Settings, secrets, the server tree, bundled runtimes, installed toolchains, and the mirrors of folders opened through SAF | App-private internal storage, `/data/data/com.kitabucode/`. No other app can read it |

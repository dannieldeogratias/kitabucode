...
This document specifies the functional and non-functional requirements for KitabuCode, a port of Visual Studio Code to Android. It serves as the contract between product vision and implementation.

### 1.2 Scope

KitabuCode is a standalone Android application that runs VS Code locally on Android devices. It consists of a Kotlin native shell, a WebView-based VS Code frontend, and a Node.js-based VS Code server backend.
...
KitabuCode is NOT a cloud IDE, a Termux wrapper, or a custom editor. It is the actual VS Code codebase running locally on the device.
...
| NFR-RES-07a | V8 heap limit (user override) | Optional, set as `kitabucode.server.heapCeilingMb`; clamped to `min(RAM / 4, 1536 MB)` and never below 256 MB | P2 |
...
| VS Code settings | App-private storage / .kitabucode/ | Sync via Settings Sync extension |

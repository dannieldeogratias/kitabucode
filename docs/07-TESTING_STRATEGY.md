...
KitabuCode has unique testing challenges: it's a hybrid app (Kotlin + WebView + Node.js) that cross-compiles native binaries. Testing must cover all layers and their integration points.
...
| KitabuCodeWebViewClient | CDN interception, `vscode-remote` resource serving, downloads | `ResourceInterceptionWiringTest`, `WebviewResourceRootsTest`, `WebviewOriginTrustTest`, `DownloadCoordinatorTest` |
...
```bash
./gradlew :app:createDebugUnitTestCoverageReport -PkitabucodeCoverage
```
...
| E2E-09 | **Copy/paste** | Copy text in Chrome → Paste in KitabuCode editor | Text pastes correctly |
...
| E2E-14 | **Package manager** | Terminal → `kitabucode pkg search curl` → `kitabucode pkg install curl` | Package installs successfully (planned Tier 3 package manager) |
...
`home/.kitabucode/data/Machine` and nothing else, so everything unnamed is already out. That makes
...
| Test | Criteria | Method |
|------|----------|--------|
| Settings backup | `home/.kitabucode/data/Machine` is included in the backup payload. **Not** `~/.kitabucode/User/`, which this row named until 2026-08-20 and which the rules have never listed; `Environment.getMachineSettingsPath` explains why the machine path is the live one | `adb backup`/device transfer test |
...
| Sensitive exclusion: connection token | `home/.kitabucode/data/token` is absent. It sits one directory from the included path, which is the near miss the allowlist exists to survive | Inspect backup payload, verify absent |

# KitabuCode Device Test Checklist
...
| SC-4 | Split-screen | Enter split-screen with another app | KitabuCode resizes correctly | | |
...
| BG-5 | Foreground notification | Check notification shade while app runs | "KitabuCode running" notification visible | | |
...
There is no Settings entry for this screen. Two routes reach it: **KitabuCode:
...
> The picker blocker ([#79](https://github.com/rmyndharis/KitabuCode/issues/79)) is
...
| ID | Scenario | Steps | Expected Result | Pass/Fail | Notes |
|----|----------|-------|-----------------|-----------|-------|
| SF-1 | Open external folder | Command palette > KitabuCode: Open Folder from Device | System picker opens; after granting, the folder appears in Explorer with its contents | | |
...
| SF-3 | Recent folders | Open a folder, close the app, reopen, run KitabuCode: Open Recent Folder | The folder is listed and reopens; its mirror was not deleted by the listing itself | | |
...
| SF-8 | Revoked permission reclaimed | Revoke the folder's access in Android Settings > Apps > KitabuCode, relaunch | The mirror is reclaimed and the device copy is untouched. A folder still granted must **not** be emptied while open | | |
| SF-9 | A save that did not reach the device | Create a file in the folder, force-stop the app immediately, relaunch and reopen the same folder | The file is present in the device folder, checked from a device file manager. Before, a write the sync never delivered stayed inside KitabuCode until the app was uninstalled, with nothing saying so | | |

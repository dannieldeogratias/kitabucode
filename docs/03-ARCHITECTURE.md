...
> those scripts and the sources under `android/app/src/main/kotlin/com/kitabucode/`, and verify
> against them.
...
KitabuCode follows a **local client-server architecture** where both the client (VS Code Web UI) and server (VS Code Server + Node.js) run on the same Android device, communicating over localhost.
...
```mermaid
flowchart TD
  USER["Developer (User)"] --> APP["KitabuCode (Android App)"]
...
**Context**: KitabuCode offers language toolchains beyond the bundled core: Ruby and Java 17. Putting them in the base APK would make it very large, and most users want at most one of them. Options: (a) On-demand download from CDN, (b) Play Store install-time asset packs, (c) Play Store on-demand asset packs.
...
- Additional languages can be added later by two routes, and there is still no Settings entry:
  ...
  **Manage toolchains**), and the command palette entry **KitabuCode: Manage Toolchains**, which
...
```mermaid
flowchart TD
  A["/data/data/com.kitabucode/"] --> B["files/"]
  B --> B1["home/ ($HOME)"]
  B1 --> B1a[".kitabucode/ (VS Code data folder)"]
  ...
  A --> D["cache/ (clearable)"]
  D --> D1["webview/"]
  E["/storage/emulated/0/Android/data/com.kitabucode/files/"] --> E1["projects/ (kept only by an install that already had it)"]
```
...
Everything the Kotlin side logs ends up in Logcat, and every tag is `KitabuCode.<class>`:
`Logger` prepends that prefix to the per-class tag it is handed, so nothing is ever logged
under the bare `KitabuCode`. Filter on a full tag, for example
`adb logcat -s KitabuCode.ProcessManager`.

| Component | Log Destination | Level |
|-----------|----------------|-------|
| Kotlin | Logcat, tag `KitabuCode.<class>` (`KitabuCode.MainActivity`, `KitabuCode.ProcessManager`, and so on) | INFO (release), DEBUG (debug) |
| Node.js server | stdout with stderr merged into it, read by `ProcessManager.startOutputReader`, which both re-logs each line as `[node] ...` under `KitabuCode.ProcessManager` and appends it to `server.log` through `ServerLog` | the Logcat copy is DEBUG, so debug builds only; the `server.log` copy is written in every build |
...
| WebView | `KitabuCodeWebChromeClient.onConsoleMessage` mirrors every console message into Logcat under `KitabuCode.WebChromeClient`; Chrome DevTools additionally attaches on debug builds | ERROR and WARNING in every build, everything else DEBUG |

`server.log` is written, `exthost.log` is not. `ProcessManager` holds a `ServerLog` over
`Environment.getLogsDir` (`filesDir/home/.kitabucode/data/logs`) and appends every line
...
|--------|----------|--------|
| VS Code settings | ~/.kitabucode/data/Machine/settings.json (the server rewrites `--user-data-dir` to `<server-data-dir>/data`. The web client takes only the remote-machine scopes from it; the server reads the same file with no scope filter, which is why an APPLICATION-scoped key it owns, `extensions.verifySignature`, does take effect) | JSON |

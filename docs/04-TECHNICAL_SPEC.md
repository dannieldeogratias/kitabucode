...
> `android/app/src/main/kotlin/com/kitabucode/`. `CONTRIBUTING.md` is the prose kept current
> alongside it.
...
name VS Code. Those nine are skipped, not failed, on a tree with no `kitabucode-patches.json`,
...
```mermaid
flowchart TD
  ROOT["com.kitabucode/"] --> APP["app/"]
  APP --> A1["KitabuCodeApp.kt (Application class)"]
  ...
  APP --> WEB["webview/"]
  WEB --> WEB1["KitabuCodeWebView.kt (WebView configuration)"]
  WEB --> WEB2["KitabuCodeWebViewClient.kt (request interception, URL loading, errors)"]
  WEB --> WEB3["KitabuCodeWebChromeClient.kt (console, file picker, permissions)"]
  ...
  RES --> DRAW["drawable/"]
  DRAW --> D1["ic_launcher.xml (KitabuCode icon)"]
...
```mermaid
flowchart TD
  A["Application.onCreate()<br/>WebView.setDataDirectorySuffix('kitabucode')<br/>Initialize logging"] --> B["SplashActivity (LAUNCHER, runs on every start)<br/>Per-launch repairs: symlinks, git core, npm wrappers, SAF reclaim<br/>Extraction only when versionName or versionCode changed<br/>Start MainActivity"]
...
val env = mapOf(
    ...
    "BASH_ENV"                to "${filesDir}/home/.kitabucode/bash-env.sh",  // see below
    "KITABUCODE_EXEC_TABLE"    to "${filesDir}/home/.kitabucode/toolchain-exec.tsv",  // see below
    ...
    "KITABUCODE_PORT"          to port.toString(),
    "KITABUCODE_VERSION"       to BuildConfig.VERSION_NAME,
)
```
...
**`KITABUCODE_EXEC_TABLE` answers exactly the cases `BASH_ENV` cannot.** It names a tab-separated
...
$NATIVE_LIB_DIR/libnode.so \
  ...
  --logsPath=$HOME/.kitabucode/data/logs \
  --log=info
```
...
Read `KitabuCodeWebView.configure` for the live set. Three notes on what is **not** here:
...
(`kitabucode://reload-editor`) loads it and clears the record, so a deliberate attempt gets
...
`kitabucode-patches.json` into the tree it produces, recording the sha256 of each patch's diff,
...
0011 walkthrough brand|out/nls.messages.js|Get Started with KitabuCode
...
ToolchainInfo(
    ...
    downloadUrl = "https://github.com/rmyndharis/KitabuCode/releases/latest/download/toolchain_ruby.zip",
)
```
...
flowchart TD
  ...
  F --> G["6. Write ~/.kitabucode/toolchain-env.sh, persist toolchains.json"]
...
Not built. No `kitabucode pkg` command exists in the app or on the device, and nothing under
...
```bash
# Search packages
kitabucode pkg search <query>

# Install package
kitabucode pkg install <package>

# List installed
kitabucode pkg list

# Remove package
kitabucode pkg remove <package>

# Update all
kitabucode pkg update
```
...
and what `scripts/verify-termux-index.sh` anchors to Termux's signing key. No KitabuCode package
host exists.
...
Mapping to VS Code version:
  ...
  KitabuCode 1.2.0 -> VS Code 1.133.0
```

### 9.2 Update Cadence

- VS Code releases monthly → KitabuCode targets monthly upstream sync
...
<data-extraction-rules>
    <cloud-backup>
        <include domain="file" path="home/.kitabucode/data/Machine" />
    </cloud-backup>
    <device-transfer>
        <include domain="file" path="home/.kitabucode/data/Machine" />
    </device-transfer>

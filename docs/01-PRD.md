...
> (`android/app/src/main/kotlin/com/kitabucode/setup/ToolchainRegistry.kt`,
...
KitabuCode is Visual Studio Code ported to Android. It provides a full-featured IDE experience on mobile devices by running the actual VS Code codebase (Code - OSS, built from the MIT source) with a native Android shell. Unlike simplified mobile code editors, KitabuCode runs real VS Code with real extension support, a real terminal, and real Node.js, delivering desktop-grade development power in your pocket.
...
| Clipboard Bridge | Copy/paste between KitabuCode and other Android apps | M2 |
...
| Package Manager | `kitabucode pkg install <package>`, not built; on the post-release roadmap | Post-release |
...
flowchart TD
  ...
  B --> C["Splash Screen: Setting up KitabuCode<br/>Extract binaries<br/>Initialize workspace directory"]
...
KitabuCode does not appear in Android's "Open with" sheet, and that is deliberate rather than
...
```mermaid
flowchart TD
  A["KitabuCode"] --> B["Command Palette:<br/>KitabuCode: Open Folder from Device"] --> C["Android SAF picker"]
...
| Feature | KitabuCode | Termux + code-server | Acode | Spck Editor |
|---------|-----------|---------------------|-------|-------------|
...
**Key differentiator**: KitabuCode = VS Code power + zero-setup + mobile UX. The only option that combines all three.
